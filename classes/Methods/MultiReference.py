...
class Multiperiod():
    ...
    def runMP(self):
        ...
        B = np.zeros((self.periods, self.num_batteries, 3))
        L = np.zeros((self.periods, self.num_batteries, 3))
        M_low = np.zeros((self.periods, self.num_batteries, 3))
        M_up = np.zeros((self.periods, self.num_batteries, 3))
        ...
        for t in range(0, self.periods):
            ...
            for idx in range(0, self.num_batteries):
                ...
                # SOC constraints
                B[t][idx][:] = infeasibility.Vsol[[bat.BtA,bat.BtB,bat.BtC]].ravel()
                L[t][idx][:] = infeasibility.Vsol[[bat.lambda_BtA,bat.lambda_BtB,bat.lambda_BtC]].ravel()
                M_low[t][idx][:] = infeasibility.Vsol[[bat.nodeA_dual_ineq_Bt,bat.nodeB_dual_ineq_Bt,bat.nodeC_dual_ineq_Bt]].ravel()
                M_up[t][idx][:] = infeasibility.Vsol[[bat.nodeA_dual_ineq_Bt_upper,bat.nodeB_dual_ineq_Bt_upper,bat.nodeC_dual_ineq_Bt_upper]].ravel()
                print(f'Period: {t}, SOC: {B[t]}')


            # Update battery object SOC in infeasibility for next iteration
            for idx in range(0, self.num_batteries):
                infeasibility.battery_sources[idx].Bt_prev = [B[t][idx][0],B[t][idx][1],B[t][idx][2]]
