...
class Store(object):
    ...
    def iter_elements(self, type=DiTToBase):
        ...
        if not issubclass(type, DiTToBase):
            raise AttributeError(
                "Unable to find {} in ditto.environment".format(type))
    ...
    def delete_disconnected_nodes(self):
        for i in self.models:
            if isinstance(i, Node) and hasattr(i,
                                               "name") and i.name is not None:
        ...
        self.build_networkx(
        )  # Should be redundant since the networkx graph is only build on connected elements
    ...
    def generate_attributes(self):
        ...
        for function_name, klass in self.model._env.get_clsmembers().items():
            ...
            setattr(self, function_name,
                    types.MethodType(f, self, EnvAttributeIntercepter))
...
def model_builder(self, klass, model, *args, **kwargs):
    ...
    cim_object = klass(init_callback=ic,
...
def get_callback(self, name, val):
    assert (self.UUID in self.link_model.cim_store
...
def set_callback(self, name, value):
    ...
    if isinstance(value, tuple):
        for v in value:
            assert (v.UUID in self.link_model.cim_store
                   ), "{} not found in Store {}".format(self.UUID,
                                                        self.link_model)
    else:
        assert (
            value.UUID
            in self.link_model.cim_store), "{} not found in Store {}".format(
                self.UUID, self.link_model)
