...
@click.option("--from", help="Convert from opendss, Cyme, gridlabd, Demo, JSON")
@click.option("--output",
              type=click.Path(exists=True),
              help="Path to the metrics output file")
...
@click.option(
    ...
    type=bool,
    help=
    "If True computes metrics per feeder. Otherwise, compute metrics at the system level.",
)
@click.pass_context
def metric(ctx, **kwargs):
    ...
    if kwargs["from"] not in registered_readers.keys():
        raise click.BadOptionUsage(
            "from", "Cannot read from format '{}'".format(kwargs["from"]))

    if kwargs["input"] is None:
        raise click.BadOptionUsage("input", "--input must be provided.")
...
@click.option("--remove_opendss_default_values",
              is_flag=True,
              help="Remove default values")
@click.option("--warehouse",
              type=click.Path(exists=True),
              help="Path to synergi warehouse file")
@click.pass_context
def convert(ctx, **kwargs):
    ...
    if kwargs["to"] not in registered_writers.keys():
        raise click.BadOptionUsage(
            "to", "Cannot write to format '{}'".format(kwargs["to"]))

    if kwargs["input"] is None:
        raise click.BadOptionUsage(
            "input", "Both --input and --output must be provided.")

    if kwargs["output"] is None:
        raise click.BadOptionUsage(
            "output", "Both --input and --output must be provided.")
