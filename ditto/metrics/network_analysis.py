    ...
    **Feeder split:**
    ...
    def __init__(self, model, compute_network=True, *args):
        ...
        if len(args) == 1:
            ...
            if len(srcs) == 0:
                ...
                raise ValueError(
                    "Multiple sourcebus found: {srcs}".format(srcs=srcs))
            else:
                source = srcs[0]
        ...
        if compute_network:
            ...
            self.edge_equipment = nx.get_edge_attributes(
                self.G.graph, "equipment")
        ...
        self.__substations = [
            obj for obj in self.model.models
    ...
    def provide_network(self, network):
        ...
        if not isinstance(network, Network):
            raise TypeError(
                "provide_network expects a Network instance. A {t} was provided."
                .format(t=type(network)))
        self.G = network
        self.G.set_attributes(self.model)
    ...
    def add_feeder_information(self, feeder_names, feeder_nodes, substations,
                               feeder_types):
        """
        ...
        if len(feeder_names) != len(feeder_nodes):
            raise ValueError(
                "Number of feeder names {a} does not match number of feeder lists of nodes {b}"
                .format(a=len(feeder_names), b=len(feeder_nodes)))
        if isinstance(feeder_types, string_types):
            ...
            if len(feeder_names) != len(feeder_types):
                raise ValueError(
                    "Number of feeder names {a} does not match number of feeder types of nodes {b}"
                    .format(a=len(feeder_names), b=len(feeder_types)))
            else:
                self.feeder_types = {
                    k: v for k, v in zip(feeder_names, feeder_types)
                }
    ...
    def export(self, *args):
        ...
        for key, data in self.results.items():
            card.loc[n_row] = [key] + [
                data[x] if x in data else None for x in cols[1:]
            ]
            n_row += 1
                ...
                if isinstance(obj, Node):
                    if obj.name in self.node_feeder_mapping:
                        ...
                        if obj.feeder_name in self.substations:
                            obj.substation_name = self.substations[
                                obj.feeder_name]
                    ...
                    if (hasattr(obj, "timeseries") and
                            obj.timeseries is not None and
                            len(obj.timeseries) > 0):
                        for t in obj.timeseries:
                            t.feeder_name = obj.feeder_name
                            t.substation_name = obj.substation_name
                        logger.debug(
                            "Object {name} connecting element {namec} was not found in feeder mapping"
                            .format(name=obj.name,
                                    namec=obj.connecting_element))
                ...
                if isinstance(obj, Node):
                    if obj.name not in self.node_feeder_mapping:
                        ...
                        while not done_looping:
                            ...
                            if (hasattr(prev_obj, "feeder_name") and
                                    hasattr(prev_obj, "name") and
                                    prev_obj.feeder_name is not None and
                                    prev_obj.feeder_name is not "" and
                                    prev_obj.name in self.
                                    node_feeder_mapping  # In case a default value has been set for all feeder_name values
                               ):
                        ...
                        logger.debug(
                            "Node {name} was not found in feeder mapping".
                            format(name=obj.name))
                    ...
                    if obj.from_element not in self.node_feeder_mapping:
                        ...
                        logger.debug(
                            "Object {name} from element {namec} was not found in feeder mapping"
                            .format(name=obj.name, namec=obj.from_element))
                else:
                    logger.debug(obj.name, type(obj))
    ...
    def setup_results_data_structure(self, *args):
        ...
        **Usage:**
            ...
            if isinstance(network, string_types):
                ...
                if network in self.feeder_names:
                    ...
                    if network in self.substations:
                        ...
                        raise ValueError(
                            "Could not find the substation for feeder {}".
                            format(network))
        ...
        results = {
            ...
            "num_reclosers":
                0,  # Number of reclosers
            "num_breakers":
                0,  # Number of breakers
            "num_capacitors":
                0,  # Number of capacitors
            "num_sectionalizers":
                0,  # Number of sectionalizers
            "num_customers":
                0,  # Number of customers
            "num_links_adjacent_feeders":
                0,  # Number of links to neighboring feeders
            "num_overloaded_transformers":
                0,  # Number of overloaded transformers
            "num_distribution_transformers":
                0,  # Number of distribution transformers
            "max_len_secondaries_mi":
                0,  # Maximum distance in the feeder between a distribution transformer and a load
            "sum_distribution_transformer_mva":
                0,  # Total capacity of distribution transformers (in MVA)
            "num_1ph_transformers":
                0,  # Number of 1 phase distribution transformers
            "num_3ph_transformers":
                0,  # Number of 3 phase distribution transformers
            "ratio_1ph_to_3ph_transformers":
                0,  # Ratio of 1 phase distribution transformers to three phase distribution transformers
            "avg_degree":
                self.average_degree(_net),  # Average degree
            "diameter":
                self.diameter(
                    _net
            ...
            "avg_regulator_sub_distance_mi":
                self.average_regulator_sub_distance(
                    _net, _src
                ),  # Average distance between substation and regulators (if any)
            "avg_capacitor_sub_distance_mi":
                self.average_capacitor_sub_distance(
                    _net, _src
                ),  # Average distance between substation and capacitors (if any)
            "avg_recloser_sub_distance_mi":
                self.average_recloser_sub_distance(
                    _net, _src
                ),  # Average distance between substation and reclosers (if any)
            "max_sub_node_distance_mi":
                self.furtherest_node_miles(_net, _src),
            "num_loops":
                self.loops_within_feeder(_net
                                        ),  # Number of loops inside the feeder
            "lv_len_mi":
                0,  # Total length of LV lines (in miles)
            "mv_len_mi":
                0,  # Total length of MV lines (in miles)
            "mv_1ph_len_mi":
                0,  # Total length of 1 phase MV lines (in miles)
            "mv_oh_1ph_len_mi":
                0,  # Total length of overhead 1 phase MV lines (in miles)
            "mv_2ph_len_mi":
                0,  # Total length of 2 phase MV lines (in miles)
            "mv_oh_2ph_len_mi":
                0,  # Total length of overhead 2 phase MV lines (in miles)
            "mv_3ph_len_mi":
                0,  # Total length of 3 phase MV lines (in miles)
            "mv_oh_3ph_len_mi":
                0,  # Total length of overhead 3 phase MV lines (in miles)
            "lv_1ph_len_mi":
                0,  # Total length of 1 phase LV lines (in miles)
            "lv_oh_1ph_len_mi":
                0,  # Total length of overhead 1 phase LV lines (in miles)
            "lv_2ph_len_mi":
                0,  # Total length of 2 phase LV lines (in miles)
            "lv_oh_2ph_len_mi":
                0,  # Total length of overhead 2 phase LV lines (in miles)
            "lv_3ph_len_mi":
                0,  # Total length of 3 phase LV lines (in miles)
            "lv_oh_3ph_len_mi":
                0,  # Total length of overhead 3 phase LV lines (in miles)
            "sum_load_kw":
                0,  # Total demand (active power)
            "sum_load_pha_kw":
                0,  # Total demand on phase A
            "sum_load_phb_kw":
                0,  # Total demand on phase B
            "sum_load_phc_kw":
                0,  # Total demand on phase C
            "sum_load_kvar":
                0,  # Total demand (reactive power)
            "num_lv_1ph_loads":
                0,  # Number of 1 phase LV loads
            "num_lv_3ph_loads":
                0,  # Number of 3 phase LV loads
            "num_mv_3ph_loads":
                0,  # Number of 3 phase MV loads
            "perct_lv_pha_load_kw":
                0,  # Percentage of total LV demand that is phase A
            "perct_lv_phb_load_kw":
                0,  # Percentage of total LV demand that is phase B
            "perct_lv_phc_load_kw":
                0,  # Percentage of total LV demand that is phase C
            "sum_lv_pha_load_kw":
                0,  # Total LV demand on phase A
            "sum_lv_phb_load_kw":
                0,  # Total LV demand on phase B
            "sum_lv_phc_load_kw":
                0,  # Total LV demand on phase C
            "avg_num_load_per_transformer":
                0,  # Average number of loads per distribution transformer
            "num_load_per_transformer": {
            },  # Store the number of loads per distribution transformer
            "num_customer_per_transformer": {
            },  # Store the number of customers per distribution transformer
            "wire_equipment_distribution": {
            },  # Store the number of each wire equipment
            "transformer_kva_distribution": [
            ],  # Store the distribution of transformer KVA values
            "ratio_load_kW_to_transformer_KVA_distribution": {
            },  # Store the ratio of load kW to distribution transformer KVA
            "switch_categories_distribution": {
            },  # Store the number of each different categories of switches
            "power_factor_distribution": [],  # Store the load poser factors
            "sub_trans_impedance_list": {
            },  # Store the list of line positive sequence impedances between the substation and each distribution transformer
            "trans_cust_impedance_list": {
            },  # Store the list of line positive sequence impedances between each customer and its distribution transformer
            "nominal_voltages": [
            ],  # Store the different nominal voltage values
            "convex_hull_area_sqmi":
                0,  # Convex hull area for the feeder
            "substation_name":
                _src,
            "feeder_type":
                None,
    ...
    def analyze_object(self, obj, feeder_name):
        ...
        if hasattr(obj, "nominal_voltage"):
            if obj.nominal_voltage not in self.results[feeder_name][
                    "nominal_voltages"]:
        ...
        if isinstance(obj, Line):
            ...
            if (hasattr(obj, "from_element") and
                ...
                try:
                    ...
                    _from = None
                    _to = None
                if (hasattr(_from, "feeder_name") and
                        _from.feeder_name is not None and
                        hasattr(_to, "feeder_name") and
                        _to.feeder_name is not None):
                    if _from.feeder_name != _to.feeder_name:
                        self.results[feeder_name][
                            "num_links_adjacent_feeders"] += 1
            ...
            if obj.is_switch == 1:
                ...
                if hasattr(obj, "nameclass") and obj.nameclass is not None:
                    if (obj.nameclass in self.results[feeder_name]
                        ["switch_categories_distribution"]):
                        self.results[feeder_name][
                            "switch_categories_distribution"][
                                obj.nameclass] += 1
                        ...
                        self.results[feeder_name][
                            "switch_categories_distribution"][obj.nameclass] = 1
            ...
            if hasattr(obj, "wires") and obj.wires is not None:
                ...
                for eq in equipment_names:
                    if eq in self.results[feeder_name][
                            "wire_equipment_distribution"]:
                        self.results[feeder_name][
                            "wire_equipment_distribution"][eq] += 1
                    else:
                        self.results[feeder_name][
                            "wire_equipment_distribution"][eq] = 1
            ...
            # If we do not have phase information, raise an error...
                ...
                raise ValueError("No phase information for line {}".format(
                    obj.name))

            # If the line is low voltage
            if (obj.nominal_voltage is not None and
                    obj.nominal_voltage * math.sqrt(3) <= self.LV_threshold):
        ...
        if isinstance(obj, Load):
            ...
            if not self.compute_kva_density_with_transformers:
                if (hasattr(obj, "transformer_connected_kva") and
                        obj.transformer_connected_kva is not None):
                    self.results[feeder_name][
                        "sum_distribution_transformer_mva"] += (
                            obj.transformer_connected_kva * 10**-6)

            if (hasattr(obj, "upstream_transformer_name") and
                    obj.upstream_transformer_name is not None):
                # Number of loads per distribution transformer
                if (obj.upstream_transformer_name in self.results[feeder_name]
                    ["num_load_per_transformer"]):
                    self.results[feeder_name]["num_load_per_transformer"][
                        obj.upstream_transformer_name] += 1
                else:
                    self.results[feeder_name]["num_load_per_transformer"][
                        obj.upstream_transformer_name] = 1
                ...
                if (hasattr(trans_obj, "to_element") and
                        trans_obj.to_element is not None):
                    _net3 = _net.copy()
                    if not _net3.has_node(trans_obj.to_element):
                        _sp = nx.shortest_path(self.G.graph,
                                               trans_obj.to_element,
                                               list(_net3.nodes())[0])
                        for n1, n2 in zip(_sp[:-1], _sp[1:]):
                            _net3.add_edge(
                                n1, n2, length=self.G.graph[n1][n2]["length"])
                    self.results[feeder_name]["trans_cust_impedance_list"][
                        obj.name] = self.get_impedance_list_between_nodes(
                            _net3, trans_obj.to_element, obj.connecting_element)

            # If the load is low voltage
            if hasattr(obj,
                       "nominal_voltage") and obj.nominal_voltage is not None:
                if obj.nominal_voltage * math.sqrt(3) <= self.LV_threshold:

                    # Update the counters
                    if hasattr(obj,
                               "phase_loads") and obj.phase_loads is not None:

                        _phase_loads_ = [
                            p for p in obj.phase_loads if p.drop != 1
                        ]
                            ...
                            if hasattr(phase_load,
                                ...
                                if (hasattr(phase_load, "p") and
                                    ...
                                    if phase_load.phase == "A":
                                        self.results[feeder_name][
                                            "sum_lv_pha_load_kw"] += phase_load.p

                                    # Phase B
                                    elif phase_load.phase == "B":
                                        self.results[feeder_name][
                                            "sum_lv_phb_load_kw"] += phase_load.p

                                    # Phase C
                                    elif phase_load.phase == "C":
                                        self.results[feeder_name][
                                            "sum_lv_phc_load_kw"] += phase_load.p

                # If the load is medium voltage
                elif (self.MV_threshold > obj.nominal_voltage * math.sqrt(3) >
                      self.LV_threshold):
                    if hasattr(obj,
                               "phase_loads") and obj.phase_loads is not None:
            ...
            if hasattr(obj, "phase_loads") and obj.phase_loads is not None:
                ...
                if np.all(np.array([pl.p for pl in _phase_loads_]) ==
                          0) and np.all(
                              np.array([pl.q for pl in _phase_loads_]) == 0):
                    if (obj.transformer_connected_kva is not None and
                            obj.transformer_connected_kva != 0):
                        self.results[feeder_name][
                            "sum_load_kw"] += obj.transformer_connected_kva
                        ...
                        for phase_load in [
                            ...
                            if hasattr(phase_load,
                                ...
                                if (hasattr(phase_load, "p") and
                                    ...
                                    if phase_load.phase == "A":
                                        self.results[feeder_name][
                                            "sum_load_pha_kw"] += float(
                                                obj.transformer_connected_kva
                                            ) / float(len(_phase_loads_))
                                    elif phase_load.phase == "B":
                                        self.results[feeder_name][
                                            "sum_load_phb_kw"] += float(
                                                obj.transformer_connected_kva
                                            ) / float(len(_phase_loads_))
                                    elif phase_load.phase == "C":
                                        self.results[feeder_name][
                                            "sum_load_phc_kw"] += float(
                                                obj.transformer_connected_kva
                                            ) / float(len(_phase_loads_))
                else:
                    ...
                    for phase_load in [
                        ...
                        if hasattr(phase_load,
                            ...
                            if hasattr(phase_load,
                                ...
                                if phase_load.phase == "A":
                                    self.results[feeder_name][
                                        "sum_load_pha_kw"] += phase_load.p
                                elif phase_load.phase == "B":
                                    self.results[feeder_name][
                                        "sum_load_phb_kw"] += phase_load.p
                                elif phase_load.phase == "C":
                                    self.results[feeder_name][
                                        "sum_load_phc_kw"] += phase_load.p
                ...
                try:
                    load_power_factor = obj.phase_loads[0].p / float(
                        math.sqrt(obj.phase_loads[0].p**2 +
                                  obj.phase_loads[0].q**2))
                    self.results[feeder_name][
                        "power_factor_distribution"].append(load_power_factor)
        ...
        if isinstance(obj, PowerTransformer):
            ...
            if obj.name in self.transformer_load_mapping:
                ...
                # Get the primary
                if hasattr(obj,
                           "from_element") and obj.from_element is not None:
                    _net2 = _net.copy()
                    if not _net2.has_node(_src):
                        _sp = nx.shortest_path(self.G.graph, _src,
                                               list(_net2.nodes())[0])
                        for n1, n2 in zip(_sp[:-1], _sp[1:]):
                            _net2.add_edge(
                                n1, n2, length=self.G.graph[n1][n2]["length"])
                ...
                if (hasattr(obj, "to_element") and
                    ...
                    for load_name in load_names:
                        try:
                            ...
                            load_obj = None
                        if (hasattr(load_obj, "connecting_element") and
                                load_obj.connecting_element is not None):
                            if self.G.graph.has_node(
                                ...
                                if (length > self.results[feeder_name]
                                    ["max_len_secondaries_mi"]):
                                    self.results[feeder_name][
                                        "max_len_secondaries_mi"] = length
                ...
                for load_name in load_names:
                    try:
                        ...
                        load_obj = None
                    if (hasattr(load_obj, "phase_loads") and
                            load_obj.phase_loads is not None):
            ...
            if (hasattr(obj, "windings") and obj.windings is not None and
                    len(obj.windings) > 0):
    ...
    def get_feeder(self, obj):
        ...
        elif (hasattr(obj, "connecting_element") and
              obj.connecting_element in self.node_feeder_mapping):
            return self.node_feeder_mapping[obj.connecting_element]
        elif (hasattr(obj, "from_element") and
              obj.from_element in self.node_feeder_mapping):
    ...
    def compute_all_metrics_per_feeder(self, **kwargs):
        ...
        for _feeder_ref in mv_feeder_names:
            total_demand_LV = (self.results[_feeder_ref]["sum_lv_pha_load_kw"] +
            ...
            if self.results[_feeder_ref]["num_customers"] != 0:
                self.results[_feeder_ref][
                    "ratio_mv_len_to_num_cust"] = self.results[_feeder_ref][
                        "mv_len_mi"] / float(
            ...
            try:
                self.results[_feeder_ref]["perct_mv_oh_len"] = (
                    ...
                    float(self.results[_feeder_ref]["lv_len_mi"] +
            ...
            if self.results[_feeder_ref]["sum_load_kw"] != 0:
                ...
                self.results[_feeder_ref]["avg_load_imbalance_by_phase"] = (
                    abs(self.results[_feeder_ref]["sum_load_pha_kw"] -
                        third_tot_demand) +
                    abs(self.results[_feeder_ref]["sum_load_phb_kw"] -
                        third_tot_demand) +
                    abs(self.results[_feeder_ref]["sum_load_phc_kw"] -
                        third_tot_demand)
                ) / self.results[_feeder_ref]["sum_load_kw"]
            else:
                self.results[_feeder_ref][
                    "avg_load_imbalance_by_phase"] = np.nan

            # Ratio of LV line length to number of customers
            if self.results[_feeder_ref]["num_customers"] != 0:
                self.results[_feeder_ref][
                    "ratio_lv_len_to_num_cust"] = self.results[_feeder_ref][
                        "lv_len_mi"] / float(
                            self.results[_feeder_ref]["num_customers"])
            else:
                self.results[_feeder_ref]["ratio_mv_len_to_num_cust"] = np.nan
            ...
            self.results[_feeder_ref][
                "avg_mv_line_impedance_sub_transformers"] = {}
            self.results[_feeder_ref][
                "max_mv_line_impedance_sub_transformers"] = {}

            for trans_name, imp_list in self.results[_feeder_ref][
                    "sub_trans_impedance_list"].items():
                if len(imp_list) > 0:
                    self.results[_feeder_ref][
                        "avg_mv_line_impedance_sub_transformers"][
                            trans_name] = np.mean(imp_list)
                    self.results[_feeder_ref][
                        "max_mv_line_impedance_sub_transformers"][
                            trans_name] = np.max(imp_list)
                else:
                    self.results[_feeder_ref][
                        "avg_mv_line_impedance_sub_transformers"][
                            trans_name] = None
            ...
            self.results[_feeder_ref][
                "avg_lv_line_impedance_transformer_cust"] = {}
            self.results[_feeder_ref][
                "max_lv_line_impedance_transformer_cust"] = {}

            for cust_name, imp_list in self.results[_feeder_ref][
                    "trans_cust_impedance_list"].items():
                if len(imp_list) > 0:
                    self.results[_feeder_ref][
                        "avg_lv_line_impedance_transformer_cust"][
                            cust_name] = np.mean(imp_list)
                    self.results[_feeder_ref][
                        "max_lv_line_impedance_transformer_cust"][
                            cust_name] = np.max(imp_list)
                else:
                    self.results[_feeder_ref][
                        "avg_lv_line_impedance_transformer_cust"][
                            cust_name] = None
            ...
            try:
                self.results[_feeder_ref][
                    "nominal_medium_voltage_class"] = np.max([
                        x for x in self.results[_feeder_ref]["nominal_voltages"]
                        if x != None
                    ])
            except:
                self.results[_feeder_ref][
                    "nominal_medium_voltage_class"] = np.nan
            ...
            if len(_points) > 2 and len(
                    unique_points) > 4:  # Ignore tiny feeders
                hull = ConvexHull(
                    _points)  # Compute the Convex Hull using Scipy
                hull_surf_sqmile = (
                    hull.area * 3.86102 * 10**-7
                )  # Convert surface from square meters to square miles
                self.results[_feeder_ref][
                    "convex_hull_area_sqmi"] = hull_surf_sqmile  # Store the convex hull area
                if hull_surf_sqmile != 0:
                    self.results[_feeder_ref]["cust_density"] = float(
                        self.results[_feeder_ref]["num_customers"]) / float(
                            hull_surf_sqmile)
                    self.results[_feeder_ref]["load_density_kw"] = float(
                        self.results[_feeder_ref]["sum_load_kw"]) / float(
                            hull_surf_sqmile)
                    self.results[_feeder_ref]["load_density_kvar"] = float(
                        self.results[_feeder_ref]["sum_load_kvar"]) / float(
                            hull_surf_sqmile)
                    self.results[_feeder_ref]["kva_density"] = float(
                        10**3 * self.results[_feeder_ref]
                        ["sum_distribution_transformer_mva"]) / float(
                            hull_surf_sqmile)
    ...
    def compute_all_metrics(self, *args, **kwargs):
        ...
        if self.results[_feeder_ref]["num_3ph_transformers"] != 0:
            self.results[_feeder_ref]["ratio_1ph_to_3ph_transformers"] = float(
                self.results[_feeder_ref]["num_1ph_transformers"]) / float(
                    self.results[_feeder_ref]["num_3ph_transformers"])
        else:
        ...
        if float(self.results[_feeder_ref]["num_reclosers"]) != 0:
            self.results[_feeder_ref][
                "num_sectionalizers_per_recloser"] = float(
                    self.results[_feeder_ref]["num_sectionalizers"]) / float(
                        self.results[_feeder_ref]["num_reclosers"])
        else:
            self.results[_feeder_ref][
                "num_sectionalizers_per_recloser"] = np.nan
        ...
        for trans_name, imp_list in self.results[_feeder_ref][
            ...
            if len(imp_list) > 0:
                ...
                self.results[_feeder_ref][
                    "avg_mv_line_impedance_sub_transformers"][trans_name] = None
                self.results[_feeder_ref][
                    "max_mv_line_impedance_sub_transformers"][trans_name] = None
        ...
        for cust_name, imp_list in self.results[_feeder_ref][
            ...
            if len(imp_list) > 0:
                ...
                self.results[_feeder_ref][
                    "avg_lv_line_impedance_transformer_cust"][cust_name] = None
                self.results[_feeder_ref][
                    "max_lv_line_impedance_transformer_cust"][cust_name] = None
    ...
    def number_of_regulators(self):
        ...
        return sum(
            [1 for obj in self.model.models if isinstance(obj, Regulator)])

    def number_of_fuses(self):
        """Returns the number of fuses."""
        return sum([
            1 for obj in self.model.models
    ...
    def number_of_capacitors(self):
        ...
        return sum(
            [1 for obj in self.model.models if isinstance(obj, Capacitor)])
            ...
            while should_continue:
                ...
                if _type == "PowerTransformer":
                    ...
                    if (from_node, end_node) in self.edge_equipment_name:
                        transformer_name = self.edge_equipment_name[(from_node,
                                                                     end_node)]
                    elif (end_node, from_node) in self.edge_equipment_name:
                        transformer_name = self.edge_equipment_name[(end_node,
                                                                     from_node)]
                    # If we cannot find the object, raise an error because it sould not be the case...
                    else:
                        raise ValueError(
                            "Unable to find equipment between {_from} and {_to}"
                            .format(_from=from_node, _to=end_node))
        ...
        for obj in self.model.models:
            if isinstance(obj, Line):
                if (hasattr(obj, "from_element") and
                    ...
                    self.node_line_mapping[(obj.from_element,
                                            obj.to_element)] = obj.name
    ...
    def get_impedance_list_between_nodes(self, net, node1, node2):
        ...
        for line in line_list:
            ...
            if (hasattr(line_object, "impedance_matrix") and
                    line_object.impedance_matrix is not None and
                    line_object.impedance_matrix != []):
    ...
    def furtherest_node_miles(self, *args):
        ...
        for node in _net.nodes():
            dist[node] = nx.shortest_path_length(_net,
                                                 _src,
                                                 node,
                                                 weight="length")
        return np.max(list(
            dist.values())) * 0.000621371  # Convert length to miles
    ...
    def furtherest_node_miles_clever(self):
        ...
        for node in self.G.graph.nodes():
            if nx.degree(self.G.graph, node) == 1:
                dist[node] = nx.shortest_path_length(self.G.graph,
                                                     self.source,
                                                     node,
                                                     weight="length")
        return np.max(list(
            ...
            if isinstance(obj, Line):
                if self.MV_threshold >= obj.nominal_voltage > self.LV_threshold:
                    if hasattr(obj, "wires") and obj.wires is not None:
                        ...
                        if (len(phases) == X and hasattr(obj, "length") and
                                obj.length >= 0):
                            total_length += obj.length
        return total_length * 0.000621371  # Convert length to miles
