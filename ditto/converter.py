...
class Converter(object):
    ...
    **Usage:**
    ...
    def __init__(self,
        ...
        if (kwargs.get("json_path", None) is not None and
                kwargs.get("registered_json_writer_class", None) is not None):
            self.jsonize = True
    ...
    def get_inputs(self, feeder):
        ...
        if self._from == "opendss":
            inputs = {
                "master_file":
                    os.path.abspath(feeder),
                "buscoordinates_file":
                    os.path.join(os.path.dirname(feeder), "buscoord.dss"),
            }
        ...
        # TODO....
            ...
            inputs = {
                "input_file_path":
                    "./inputs/{format}/{feeder}/{feeder}.dew".format(
                        format=self._from, feeder=feeder),
                "databasepath":
                    "../readers/dew/DataBase/DataBase.xlsx",
            }

        else:
            raise NotImplementedError("Format {} not imlemented.".format(
                self._from))
    ...
    def configure_reader(self, inputs):
        ...
        logger.debug("Using Reader {} with inputs {}".format(
            self.reader_class, inputs))
    ...
    def configure_writer(self, outputs):
        ...
        if self.writer_class is not None:
            logger.debug("Using Writer {} with outputs {}".format(
                self.writer_class, outputs))
            self.writer = self.writer_class(**outputs)
        else:
            logger.error(
                "Cannot configure the writer because Writer class is None.")
    ...
    def convert(self):
        ...
        self.current_time_string = datetime.datetime.now().strftime(
            self.time_format)
        ...
        if self.jsonize:
            self.json_writer = self.json_writer_class(
