...
from ditto.readers.synergi.length_units import (SynergiValueType,
                                                convert_length_unit)
    ...
    **Contributor:**
    ...
    def get_data(self, key1, key2):
        ...
        **Output:**
        ...
        """
        if (key1 in self.SynergiData.SynergiDictionary and
                key2 in self.SynergiData.SynergiDictionary[key1]):
            return self.SynergiData.SynergiDictionary[key1][key2]
        else:
            print("Could not retrieve data for <{k1}><{k2}>.".format(k1=key1,
                                                                     k2=key2))
            return None
    ...
    def parse(self, model):
        ...
        if self.ware_house_input_file is not None:
            ...
            self.SynergiData = DbParser(self.input_file,
                                        warehouse=self.ware_house_input_file)
        else:
            self.SynergiData = DbParser(self.input_file)
        ...
        SubstationId = self.get_data("InstFeeders",
                                     "SubstationId")  # wenbo added
        ...
        PosSequenceResistance_src = self.get_data("InstFeeders",
                                                  "PosSequenceResistance")
        PosSequenceReactance_src = self.get_data("InstFeeders",
                                                 "PosSequenceReactance")
        ZeroSequenceResistance_src = self.get_data("InstFeeders",
                                                   "ZeroSequenceResistance")
        ZeroSequenceReactance_src = self.get_data("InstFeeders",
        ...
        self.get_data("InstSection", "AveHeightAboveGround_MUL")
        ...
        for idx, id in enumerate(FeederId):
            self.feeder_substation_mapping[FeederId[idx].replace(
                " ", "_")] = SubstationId[idx].replace(" ", "_")
        ...
        FeederId = FeederId.append(pd.Series(FeederId_subtrans),
                                   ignore_index=True)

        FromNodeId = self.get_data("InstSection", "FromNodeId")
        ToNodeId = self.get_data("InstSection", "ToNodeId")
        self.get_data("InstSection", "IsFromEndOpen")
        self.get_data("InstSection", "IsToEndOpen")
        self.get_data("InstSection", "AmpRating")
        AveHeightAboveGround_MUL = self.get_data("InstSection",
                                                 "AveHeightAboveGround_MUL")
        ...
        self.section_feeder_mapping = create_mapping(LineID,
                                                     LineFeederId,
                                                     remove_spaces=True)
        ...
        for idx, section in enumerate(LineID):
            self.section_from_to_mapping[section] = (FromNodeId[idx],
                                                     ToNodeId[idx])

        ###### Transformer ##################
        TransformerId = self.get_data("InstPrimaryTransformers",
                                      "UniqueDeviceId")
        TransformerSectionId = self.get_data("InstPrimaryTransformers",
                                             "SectionId")
        TransformerType = self.get_data("InstPrimaryTransformers",
                                        "TransformerType")

        self.get_data("InstDTrans", "DTranId")
        self.get_data("InstDTrans", "SectionId")
        self.get_data("InstDTrans", "HighSideConnCode")
        self.get_data("InstDTrans", "LowSideConnCode")
        self.get_data("InstDTrans", "ConnPhases")

        ## Substration Transformers ##
        # wenbo added
        SubstationTransformerV = self.get_data("InstSubstationTransformers",
                                               "NominalKvll")
        ...
        NominalKvll_src = NominalKvll_src.append(SubstationTransformerV,
                                                 ignore_index=True)
        ...
        SubtransByPhVoltDegPh1 = self.get_data("InstSubstationTransformers",
                                               "ByPhVoltDegPh1")
        ByPhVoltDegPh1 = ByPhVoltDegPh1.append(SubtransByPhVoltDegPh1,
                                               ignore_index=True)

        ## Transformer Setting ##

        TransformerTypesinStock = self.get_data("DevTransformers",
                                                "TransformerName")
        HighSideRatedKv = self.get_data("DevTransformers", "HighSideRatedKv")
        LowSideRatedKv = self.get_data("DevTransformers", "LowSideRatedKv")
        TransformerRatedKva = self.get_data("DevTransformers",
                                            "TransformerRatedKva")
        EmergencyKvaRating = self.get_data("DevTransformers",
                                           "EmergencyKvaRating")
        IsThreePhaseUnit = self.get_data("DevTransformers", "IsThreePhaseUnit")
        self.get_data("DevTransformers", "NoLoadLosses")
        ...
        PercentResistance = self.get_data("DevTransformers",
                                          "PercentResistance")
        self.get_data("InstPrimaryTransformers", "ConnectedPhases")
        ...
        HighVoltageConnectionCode_N = self.get_data("InstPrimaryTransformers",
                                                    "HighSideConnectionCode")
        ...
        LowVoltageConnectionCode_N = self.get_data("InstPrimaryTransformers",
                                                   "LowSideConnectionCode")
        # Low side connection code from warehouse
        LowVoltageConnectionCode_W = self.get_data("DevTransformers",
                                                   "LowVoltageConnectionCode")

        # Tertiary connection code from network
        TertConnectCode = self.get_data("InstPrimaryTransformers",
                                        "TertConnectCode")
        # Tertiary connection code from warehouse
        TertiaryConnectionCode = self.get_data("DevTransformers",
                                               "TertiaryConnectionCode")

        ## Reclosers #######
        recloser_sectionID = self.get_data("InstReclosers", "SectionId")
        self.get_data("InstReclosers", "UniqueDeviceId")
        recloser_rating = self.get_data("InstReclosers", "AmpRating")
        RecloserIsOpen = self.get_data("InstReclosers", "RecloserIsOpen")
        recloser_interrupting_rating = self.get_data("InstReclosers",
                                                     "InterruptRatingAmps")

        ## Switches ########
        switch_sectionID = self.get_data("InstSwitches", "SectionId")
        self.get_data("InstSwitches", "UniqueDeviceId")
        ...
        EmergencyCurrentRating_switch = self.get_data("DevSwitches",
                                                      "EmergencyCurrentRating")

        ## Fuses #########
        fuse_sectionID = self.get_data("InstFuses", "SectionId")
        self.get_data("InstFuses", "UniqueDeviceId")
        fuse_rating = self.get_data("InstFuses", "AmpRating")
        fuse_blow_rating = self.get_data("InstFuses", "CutoffAmps")
        self.get_data("InstFuses", "ConnectedPhases")
        fuse_is_open = self.get_data("InstFuses", "FuseIsOpen")

        ## Protective devices ############
        protective_device_sectionID = self.get_data("InstProtectiveDevices",
                                                    "SectionId")
        protective_device_deviceID = self.get_data("InstProtectiveDevices",
                                                   "UniqueDeviceId")
        protective_device_connected_phases = self.get_data(
            "InstProtectiveDevices", "ConnectedPhases")
        ProtectiveDeviceTypeName = self.get_data("DevProtectiveDevices",
                                                 "ProtectiveDeviceTypeName")
        ProtectiveDeviceType = self.get_data("DevProtectiveDevices",
        ...
        CableDiamConductor = self.get_data("DevConductors",
                                           "Diameter_SUL")  # wenbo changed
        CableResistance = self.get_data("DevConductors",
        ...
        ContinuousCurrentRating = self.get_data("DevConductors",
                                                "ContinuousCurrentRating")
        InterruptCurrentRating = self.get_data("DevConductors",
        ...
        CableConNeutStrandCount = self.get_data("DevConductors",
                                                "CableConNeutStrandCount")
        CableDiamOutside = self.get_data("DevConductors",
                                         "CableDiamOutside_SUL")
        CableDiamOverInsul = self.get_data("DevConductors",
                                           "CableDiamOverInsul_SUL")
        ...
        for idx, cond in enumerate(ConductorName):
            conductor_mapping[cond] = {
                "CableGMR":
                    CableGMR[idx],
                "CableDiamConductor":
                    CableDiamConductor[idx],
                "CableResistance":
                    CableResistance[idx],
                "PosSequenceResistance_PerLUL":
                    PosSequenceResistance_PerLUL[idx],
                "PosSequenceReactance_PerLUL":
                    PosSequenceReactance_PerLUL[idx],
                "ZeroSequenceResistance_PerLUL":
                    ZeroSequenceResistance_PerLUL[idx],
                "ZeroSequenceReactance_PerLUL":
                    ZeroSequenceReactance_PerLUL[idx],
                "ContinuousCurrentRating":
                    ContinuousCurrentRating[idx],
                "InterruptCurrentRating":
                    InterruptCurrentRating[idx],
                "CableConNeutStrandDiameter_SUL":
                    CableConNeutStrandDiameter_SUL[idx],
                "CableConNeutResistance_PerLUL":
                    CableConNeutResistance_PerLUL[idx],
                "CableConNeutStrandCount":
                    CableConNeutStrandCount[idx],
                "CableDiamOutside":
                    CableDiamOutside[idx],
                "CableDiamOverInsul":
                    CableDiamOverInsul[idx],
            }
        ...
        CapacitorConnectionType = self.get_data("InstCapacitors",
                                                "ConnectionType")
        CapacitorTimeDelaySec = self.get_data("InstCapacitors", "TimeDelaySec")
        CapacitorPrimaryControlMode = self.get_data("InstCapacitors",
                                                    "PrimaryControlMode")
        ...
        CapacitorSectionId = self.get_data("InstCapacitors", "SectionId")
        CapacitorFixedKvarPhase1 = self.get_data("InstCapacitors",
                                                 "FixedKvarPhase1")
        CapacitorFixedKvarPhase2 = self.get_data("InstCapacitors",
                                                 "FixedKvarPhase2")
        CapacitorFixedKvarPhase3 = self.get_data("InstCapacitors",
                                                 "FixedKvarPhase3")
        CapacitorModule1KvarPhase = self.get_data("InstCapacitors",
                                                  "Module1KvarPerPhase")
        MeteringPhase = self.get_data("InstCapacitors", "MeteringPhase")
        CapacitorConnectedPhases = self.get_data("InstCapacitors",
                                                 "ConnectedPhases")

        ## Regulators ###################
        RegulatorId = self.get_data("InstRegulators", "UniqueDeviceId")
        RegulatorTimeDelay = self.get_data("InstRegulators", "TimeDelaySec")
        RegulatorTapLimiterHighSetting = self.get_data("InstRegulators",
                                                       "TapLimiterHighSetting")
        RegulatorTapLimiterLowSetting = self.get_data("InstRegulators",
                                                      "TapLimiterLowSetting")
        RegulatorTapLimiterLowSetting = self.get_data("InstRegulators",
                                                      "TapLimiterLowSetting")
        RegulatrorForwardBWDialPhase1 = self.get_data("InstRegulators",
                                                      "ForwardBWDialPhase1")
        RegulatrorForwardBWDialPhase2 = self.get_data("InstRegulators",
                                                      "ForwardBWDialPhase2")
        RegulatrorForwardBWDialPhase3 = self.get_data("InstRegulators",
                                                      "ForwardBWDialPhase3")
        ...
        self.get_data("InstRegulators", "NearFromNode")
        RegulatorTapsNearFromNode = self.get_data(
            "InstRegulators", "TapsNearFromNode")  # wenbo added

        RegulatorRatedVoltage = self.get_data("DevRegulators",
                                              "RegulatorRatedVoltage")
        RegulatorRatedKva = self.get_data("DevRegulators", "RegulatorRatedKva")
        self.get_data("DevRegulators", "NoLoadLosses")
        RegulatorConnectionCode = self.get_data("DevRegulators",
                                                "ConnectionCode")

        # wenbo added
        RegulatorPercentZ = self.get_data("DevRegulators",
                                          "PercentZOnRegulatorBase")
        ...
        LargeCustLoadPhase1Kvar = self.get_data("InstLargeCust",
                                                "LoadPhase1Kvar")
        LargeCustLoadPhase2Kvar = self.get_data("InstLargeCust",
                                                "LoadPhase2Kvar")
        LargeCustLoadPhase3Kvar = self.get_data("InstLargeCust",
                                                "LoadPhase3Kvar")
        ...
        for i, x in enumerate(LargeCustType):
            if x.upper() == "L" or x.upper() == "C":
                LoadName = LoadName.append(pd.Series(LargeCustDeviceId[i]),
                                           ignore_index=True)
                Phase1Kw = Phase1Kw.append(pd.Series(LargeCustLoadPhase1Kw[i]),
                                           ignore_index=True)
                Phase2Kw = Phase2Kw.append(pd.Series(LargeCustLoadPhase2Kw[i]),
                                           ignore_index=True)
                Phase3Kw = Phase3Kw.append(pd.Series(LargeCustLoadPhase3Kw[i]),
                                           ignore_index=True)
                Phase1Kvar = Phase1Kvar.append(pd.Series(
                    LargeCustLoadPhase1Kvar[i]),
                                               ignore_index=True)
                Phase2Kvar = Phase2Kvar.append(pd.Series(
                    LargeCustLoadPhase2Kvar[i]),
                                               ignore_index=True)
                Phase3Kvar = Phase3Kvar.append(pd.Series(
                    LargeCustLoadPhase3Kvar[i]),
                                               ignore_index=True)
        ...
        GeneratorConnectedPhases = self.get_data("InstGenerators",
                                                 "ConnectedPhases")
        self.get_data("InstGenerators", "MeteringPhase")
        GeneratorType = self.get_data("InstGenerators", "GeneratorType")
        self.get_data("InstGenerators", "VoltageSetting")
        ...
        self.get_data("DevGenerators", "PercentPFRating")
        ...
        for i, obj in enumerate(FeederId):
            ...
            api_source.nominal_voltage = NominalKvll_src[
                i] * 10**3  # DiTTo in volts

            # Set the per unit
            api_source.per_unit = (BusVoltageLevel[i] / 120.0
        ...
        for i, obj in enumerate(LineID):
            ...
            if ConfigurationId is not None:
                if (isinstance(ConfigurationId[i], str) and
                        len(ConfigurationId[i]) > 0 and
                        ConfigurationId[i] in config_mapping):
            ...
            #
            api_line.length = convert_length_unit(LineLength[i],
            ...
            if recloser_sectionID is not None and obj in recloser_sectionID.values:
                ...
                if len(idd) == 1:
                    eqt_interrupting_rating = recloser_interrupting_rating[
                        idd[0]]
            ...
            # Protection Devices
            if (protective_device_sectionID is not None and
                    obj in protective_device_sectionID.values):
                idd = np.argwhere(
                    protective_device_sectionID.values == obj).flatten()

                # Get the type of protector
                if len(idd) == 1:

                    if (protective_device_deviceID[idd[0]]
                            in ProtectiveDeviceTypeName.values):
                        eqt_id = np.argwhere(
                            ProtectiveDeviceTypeName.values ==
                            protective_device_deviceID[idd[0]]).flatten()
                        ...
                        if len(eqt_id) == 1:
                            ...
                            eqt_interrupting_rating = InterruptCurrentRating[
                                eqt_id]
            ...
            for idx, phase in enumerate(SectionPhases_thisline):
                ...
                if api_line.is_recloser == 1:
                    ...
                    api_wire.ampacity = float(
                        eqt_rating)  # Value should already be in amps
                ...
                if phase != "N":
                    ...
                    if (idx == 0 and phase != "N" and
                            "Position1_X_MUL" in config and
                            "Position1_Y_MUL" in config):
                        # Set X
                        ...
                        api_wire.Y = (AveHeightAboveGround_MUL[i] +
                                      config["Position1_Y_MUL"])
                    ...
                    if (idx == 1 and phase != "N" and
                            "Position2_X_MUL" in config and
                            "Position2_Y_MUL" in config):
                        # Set X
                        #                        api_wire.X = convert_length_unit(
                    ...
                    if (idx == 2 and phase != "N" and
                            "Position3_X_MUL" in config and
                            "Position3_Y_MUL" in config):
                        # Set X
                    ...
                    if (idx == 0 and PhaseConductorID is not None and
                            isinstance(PhaseConductorID[i], str) and
                            len(PhaseConductorID[i]) > 0):
                    ...
                    #
                    if idx == 1:
                        if (PhaseConductor2Id is not None and
                                isinstance(PhaseConductor2Id[i], str) and
                                len(PhaseConductor2Id[i]) > 0 and
                                PhaseConductor2Id[i].lower() != "unknown"):
                            # Set the nameclass
                            # Replace spaces with "_"
                            #
                            api_wire.nameclass = PhaseConductor2Id[i].replace(
                                " ", "_")
                            ...
                            try:
                                api_wire.nameclass = PhaseConductorID[
                                    i].replace(" ", "_")
                                conductor_name_raw = PhaseConductorID[i]
                            except:
                                pass
                    ...
                    if idx == 2:
                        if (PhaseConductor3Id is not None and
                                isinstance(PhaseConductor3Id[i], str) and
                                len(PhaseConductor3Id[i]) > 0 and
                                PhaseConductor3Id[i].lower() != "unknown"):
                            # Set the nameclass
                            # Replace spaces with "_"
                            #
                            api_wire.nameclass = PhaseConductor3Id[i].replace(
                                " ", "_")
                ...
                if phase == "N":
                    if (NeutralConductorID is not None and
                            isinstance(NeutralConductorID[i], str) and
                            len(NeutralConductorID[i]) > 0):

                        # Set the nameclass
                        api_wire.nameclass = NeutralConductorID[i].replace(
                            " ", "_")
                        # Cache the conductor name
                ...
                # - Resistance
                #
                if (conductor_name_raw is not None and
                        conductor_name_raw in conductor_mapping):
                    ...
                    if api_wire.ampacity is None:
                        api_wire.ampacity = conductor_mapping[
                            conductor_name_raw]["ContinuousCurrentRating"]
                    ...
                    if api_wire.emergency_ampacity is None:
                        api_wire.emergency_ampacity = conductor_mapping[
                            conductor_name_raw]["InterruptCurrentRating"]
                    ...
                    if (conductor_mapping[conductor_name_raw]
                        ["CableDiamOutside"] >
                            conductor_mapping[conductor_name_raw]
                        ["CableDiamConductor"]):
                        api_wire.concentric_neutral_resistance = (
                            conductor_mapping[conductor_name_raw]
                            ["CableConNeutResistance_PerLUL"] / 160934)
                        api_wire.concentric_neutral_diameter = (
                            conductor_mapping[conductor_name_raw]
                            ["CableConNeutStrandDiameter_SUL"] * 0.0254
                        )  # multiplied by short unit length scale
                        api_wire.concentric_neutral_gmr = (
                            conductor_mapping[conductor_name_raw]
                            ["CableConNeutStrandDiameter_SUL"] / 2.0 * 0.7788 *
                            0.0254
            ...
            if Count is not None:
                ...
                r0 = convert_length_unit(r0, SynergiValueType.Per_LUL,
                                         LengthUnits) / 3
                r1 = convert_length_unit(r1, SynergiValueType.Per_LUL,
                                         LengthUnits) / 3
                x0 = convert_length_unit(x0, SynergiValueType.Per_LUL,
                                         LengthUnits) / 3
                x1 = convert_length_unit(x1, SynergiValueType.Per_LUL,
                                         LengthUnits) / 3
                B1 = (convert_length_unit(B1, SynergiValueType.Per_LUL,
                                          LengthUnits) / 3 / 376.9911)
                B0 = (convert_length_unit(B0, SynergiValueType.Per_LUL,
                                          LengthUnits) / 3 / 376.9911)
        ...
        for i, obj in enumerate(TransformerId):
            ...
            # wenbo changed this
            api_transformer.name = obj.replace(",", "").replace(" ",
                                                                "_").lower()
            ...
            if Count is not None:
                if api_transformer.HighSideNearFromNode == 1:
                    api_transformer.to_element = (ToNodeId[Count].replace(
                        " ", "_").lower())
                else:
                    api_transformer.to_element = (FromNodeId[Count].replace(
                        " ", "_").lower())

            # Set the From element
            if Count is not None:
                if api_transformer.HighSideNearFromNode == 1:
                    api_transformer.from_element = (FromNodeId[Count].replace(
                        " ", "_").lower())
                else:
                    api_transformer.from_element = (ToNodeId[Count].replace(
                        " ", "_").lower())
            ...
            if Count is None:
                print("WARNING: No transformer equipment found for section {}".
                      format(obj))
            ...
            if Count is not None:
                ...
                api_transformer.reactances = [
                    math.sqrt(PercentImpedance[Count]**2 -
                              PercentResistance[Count]**2)
                ]
                ...
                # Get the phases:
                phases = self.section_phase_mapping[
                    TransformerSectionId[i]].replace(" ", "")
                ...
                for winding in range(n_windings):
                    ...
                    if winding == 0:
                        ...
                        if (HighVoltageConnectionCode_W is not None and
                                len(HighVoltageConnectionCode_W[i]) > 0):
                            w.connection_type = HighVoltageConnectionCode_N[
                                i][:1].upper()
                        ...
                        w.nominal_voltage = (HighSideRatedKv[Count] * 10**3
                                            )  # DiTTo in volts
                    ...
                    # Secondary
                        ...
                        # Set the Connection_type of the Winding
                        if (LowVoltageConnectionCode_W is not None and
                                len(LowVoltageConnectionCode_W[i]) > 0):
                            w.connection_type = LowVoltageConnectionCode_W[
                                i][:1].upper()
                        elif LowVoltageConnectionCode_N is not None:
                            w.connection_type = LowVoltageConnectionCode_N[
                                Count][:1].upper()

                        # Set the Nominal voltage of the Winding
                        w.nominal_voltage = (LowSideRatedKv[Count] * 10**3
                                            )  # DiTTo in volts
                    ...
                    # Tertiary
                        ...
                        # Set the Connection_type of the Winding
                        if TertConnectCode is not None and len(
                                TertConnectCode[i]) > 0:
                            w.connection_type = TertConnectCode[i][:1].upper()
                        elif TertiaryConnectionCode is not None:
                            w.connection_type = TertiaryConnectionCode[
                                Count][:1].upper()

                        # Set the Nominal voltage of the Winding
                        w.nominal_voltage = (TertiaryRatedKv[Count] * 10**3
                                            )  # DiTTo in volts
                    ...
                    if winding == 0 or winding == 1:
                        ...
                        w.rated_power = (TertiaryKva * 10**3
        ...
        for i, obj in enumerate(LoadName):
            ...
            if Count is not None:
                ...
                api_load.connecting_element = ToNodeId[Count].lower().replace(
                    " ", "_")
                ...
                PLoad = map(lambda x: x * 10**3,
                            [Phase1Kw[i], Phase2Kw[i], Phase3Kw[i]])

                QLoad = map(lambda x: x * 10**3,
                            [Phase1Kvar[i], Phase2Kvar[i], Phase3Kvar[i]])
                ...
                if (len(api_load.phase_loads) == 1 and len(
                        self.node_nominal_voltage_mapping.get(
                            api_load.connecting_element)[1]) == 4):

                    api_load.nominal_voltage = round(
                        self.node_nominal_voltage_mapping.get(
                            api_load.connecting_element)[0] / 1.732,
                        1,
                    )

                elif (len(api_load.phase_loads) > 1 and len(
                        self.node_nominal_voltage_mapping.get(
                            api_load.connecting_element)[1]) == 3 and
                      self.node_nominal_voltage_mapping.get(
                          api_load.connecting_element)[1][-1] == "N"):
                    api_load.nominal_voltage = round(
                        self.node_nominal_voltage_mapping.get(
                            api_load.connecting_element)[0] * 1.732,
                        1,
        ...
        for i, obj in enumerate(CapacitorName):
            ...
            if CapacitorSectionID[i] in self.section_feeder_mapping:
                api_cap.feeder_name = self.section_feeder_mapping[
                    CapacitorSectionID[i]]
            ...
            if CapacitorPrimaryControlMode[i] in control_mode_mapping:
                api_cap.mode = control_mode_mapping[
                    CapacitorPrimaryControlMode[i]]
            ...
            if Count is not None:
                ...
                api_cap.connecting_element = ToNodeId[Count].lower().replace(
                    " ", "_")
        ...
        for i, obj in enumerate(RegulatorId):
            ...
            if len(bandwidths) != 0:
                ...
                # Otherwise, use the smallest one and print a warning...
                    ...
                    print(
                        "WARNING: DiTTo does not support different bandwidth values on different phase. Got {ba} for A, {bb} for B, and {bc} for C. Using {x}..."
                        .format(
            ...
            if len(bandcenters) != 0:
                ...
                # Otherwise, use the smallest one and print a warning...
                    ...
                    print(
                        "WARNING: DiTTo does not support different bandcenter values on different phase. Got {ba} for A, {bb} for B, and {bc} for C. Using {x}..."
                        .format(
            ...
            if Count is not None:
                ...
                api_regulator.xhl = (np.sqrt(RegulatorPercentZ[Count]**2 /
                                             (RegulatorXRRatio[Count]**2 + 1)) *
            ...
            for winding in range(n_windings):
                ...
                if Count is not None:
                    ...
                    w.connection_type = RegulatorConnectionCode[
                        Count][:1].upper()
            ...
            if Count is not None:
                ...
                # Set the from element
                api_regulator.from_element = FromNodeId[Count].lower().replace(
                    " ", "_")

                # Set the to element
                api_regulator.to_element = ToNodeId[Count].lower().replace(
                    " ", "_")
        ...
        for i, obj in enumerate(PVUniqueDeviceId):
            ...
            if PVGenType[i] == "photovoltaic" or PVGenType[
                    i] == "photovoltaic 3p":
                ...
                if Count is None:
                    ...
                    # Set the connecting element
                    api_PV.connecting_element = ToNodeId[Count].replace(
                        " ", "_")

                # from the connecting element, get the nominal voltage
                if (len(api_PV.phases) == 1 and len(
                        self.node_nominal_voltage_mapping.get(
                            api_PV.connecting_element)[1]) == 4):
                    api_PV.nominal_voltage = round(
                        self.node_nominal_voltage_mapping.get(
                            api_PV.connecting_element)[0] / 1.732,
                        1,
                    )
                elif (len(api_PV.phases) > 1 and len(
                        self.node_nominal_voltage_mapping.get(
                            api_PV.connecting_element)[1]) == 3 and
                      self.node_nominal_voltage_mapping.get(
                          api_PV.connecting_element)[1][-1] == "N"):
                    api_PV.nominal_voltage = round(
                        self.node_nominal_voltage_mapping.get(
                            api_PV.connecting_element)[0] * 1.732,
                        1,
                    ...
                    api_PV.nominal_voltage = self.node_nominal_voltage_mapping.get(
                        api_PV.connecting_element)[0]

        # check generator tab for PVs
        for idx, obj in enumerate(GeneratorSectionID):
            Count = None

            if (GeneratorType[idx] == "photovoltaic" or GeneratorType[idx]
                    == "photovoltaic 3p"):  # this is for non-special name PV
                ...
                if GenPhase1Kvar[idx] != 0:
                    api_PV.phases.append(
                        "A") if "A" not in api_PV.phases else None
                    reactive_rating_pv += GenPhase1Kvar[idx]
                if GenPhase2Kvar[idx] != 0:
                    api_PV.phases.append(
                        "B") if "B" not in api_PV.phases else None
                    reactive_rating_pv += GenPhase2Kvar[idx]
                if GenPhase3Kvar[idx] != 0:
                    api_PV.phases.append(
                        "C") if "C" not in api_PV.phases else None
            ...
            if (GeneratorType[idx] != "photovoltaic" and GeneratorType[idx] !=
                    "photovoltaic 3p"):  # this is for non-special name PV
                ...
                for k, gen in enumerate(GeneratorName):
                    ...
                    if Count is not None and GeneratorTypeDev[Count].lower(
                    ) == "pv":
                        ...
                        if Count2 is None:
                            print("WARNING: No section found for PV {}".format(
                                obj))
                        ...
                        if len(api_PV.phases) == 1:
                            ...
                            api_PV.nominal_voltage = GeneratorKvRating[
                                Count] * 10**3
