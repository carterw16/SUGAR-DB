            ...
            if isinstance(object_dictionary, list):
                for ElmDict in object_dictionary:
                    if '@objectID' in ElmDict:
                        ...
                        self.elements_by_class[object_name][
                            ElmDict['@objectID']] = ElmDict
            else:
                if '@objectID' in object_dictionary:
                    self.elements[
                        object_dictionary['@objectID']] = object_dictionary
                    self.elements_by_class[object_name][
                        object_dictionary['@objectID']] = object_dictionary
        return
            ...
            if '.xml' in filename:
                try:
                    ...
                    XmlDict = xmltodict.parse(
                        XmlFile.read())['MultiSpeakMsgHeader']['MultiSpeak']
                    XmlFile.close()
                    self.__CreateElmDicts(XmlDict)
                except:
                    raise ValueError(
                        'Unable to open file {name}'.format(name=filename))

        # The code is used to read all the relevant equipment libraries and create a dictionary of DataFrames
        self.libraries = {}
        for library_name, library_path in self.windmil_folders[
                'Libraries'].items():
            try:
                ...
                raise ValueError(
                    'Unable to open file {name}'.format(name=library_path))
        return
    ...
    def parse_nodes(self, model):
        ...
        for element_name, element_properties in self.elements.items():
            if 'parentSectionID' in element_properties:
                node_from = self.fixStr(
                    element_properties['parentSectionID']['@name'])
