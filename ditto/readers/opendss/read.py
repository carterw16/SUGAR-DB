    ...
    **Constructor:**
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs.get("default_values_file", None) is not None:
            self.DSS_file_names["default_values_file"] = kwargs[
                "default_values_file"]
        else:
    ...
    def phase_mapping(self, dss_phase):
        ...
        **Phase mapping:**
        ...
        if dss_phase == 3 or dss_phase == "3" or dss_phase == "C":
            return "C"
        if dss_phase == 4 or dss_phase == 0 or dss_phase == '0' or dss_phase == "4" or dss_phase == "N":
            return "N"
        return None
    ...
    def build_opendssdirect(self, master_dss_file):
        ...
        try:
            self.function(
                "redirect {master_file}".format(master_file=master_dss_file))
        except:
            logger.error("Unable to redirect master file: {filename}".format(
    ...
    def parse(self, model, **kwargs):
        ...
        self.parse_reactors(model)

        self.parse_photovoltaics(model)
        ...
        self.all_api_objects["breakers"] = self._breakers
        self.all_api_objects["reclosers"] = self._reclosers
        self.all_api_objects["switches"] = self._switches
        self.all_api_objects["reactors"] = self._reactors
    ...
    def set_nominal_voltages(self, model):
        ...
        for bus_name in AllBusNames:
            ...
            try:
                model[bus_name.lower()].nominal_voltage = (
                    dss.Bus.kVBase() * math.sqrt(3) * 10**3)  # DiTTo in volts
                model[bus_name.lower()].per_unit = dss.Bus.puVmagAngle()[0]
            except:
                print("Could not set nominal voltage for bus {b}".format(
                    b=bus_name))
        ...
        for obj in model.models:
            if hasattr(obj, "nominal_voltage") and obj.nominal_voltage is None:
                ...
                if hasattr(obj, "connecting_element"):
                    try:
                        obj.nominal_voltage = model[
                            obj.connecting_element].nominal_voltage
                    except:
                        pass
                elif hasattr(obj, "from_element"):
                    try:
                        obj.nominal_voltage = model[
                            obj.from_element].nominal_voltage
                    except:
                        pass
            elif isinstance(obj, PowerTransformer) or isinstance(
                    obj, Regulator):
                ...
                for x in range(3):
                    if (len(obj.windings) > x and
                            obj.windings[x].nominal_voltage is None):
                        try:
                            obj.windings[x].nominal_voltage = model[
                                mapp[x]].nominal_voltage
                        except:
                            pass
    ...
    def parse_feeder_metadata(self, model):
        ...
        for f_name, f_data in feeders.items():
            ...
            if f_name in substation_transformers:
                api_feeder_metadata.transformer = substation_transformers[
                    f_name]
        ...
        for source_name, source_data in sources.items():
            ...
            try:
                api_power_source.nominal_voltage = (
                    float(source_data["basekv"]) * 10**3)  # DiTTo in volts
            except:
                pass
            ...
            try:
                api_power_source.rated_power = float(
                    source_data["baseMVA"]) * 10**6
            except:
                pass
                ...
                if len(source_data["Z0"]) == 2:
                    try:
                        api_power_source.zero_sequence_impedance = complex(
                            float(source_data["Z0"][0]),
                            float(source_data["Z0"][1]))
                    except:
                        pass
            elif ("R0" in source_data and source_data["R0"] != "" and
                  "X0" in source_data and source_data["X0"] != ""):
                try:
                    api_power_source.zero_sequence_impedance = complex(
                ...
                if len(source_data["Z1"]) == 2:
                    try:
                        api_power_source.positive_sequence_impedance = complex(
                            float(source_data["Z1"][0]),
                            float(source_data["Z1"][1]))
                    except:
                        pass
            elif ("R1" in source_data and source_data["R1"] != "" and
                  "X1" in source_data and source_data["X1"] != ""):
                try:
                    api_power_source.positive_sequence_impedance = complex(
            ...
            if not skip_coordinate_parsing:
                for line in coordinates:
                    ...
                    try:
                        ...
                        logger.warning(
                            "Could not cast coordinates {X}, {Y} for bus {name}"
            ...
            try:
                if "." in source_data["bus1"]:
                    api_power_source.connecting_element = source_data[
                        "bus1"].split(".")[0]
                else:
            ...
            self._slacks[api_power_source.connecting_element] = api_power_source
    ...
    def parse_nodes(self, model, **kwargs):
        ...
        for name, data in lines.items():
            ...
            if b1_name is not None and not b1_name in buses:
                ...
                buses[b1_name]["phases"] += b1_phases
                buses[b1_name]["phases"] = np.unique(
                    buses[b1_name]["phases"]).tolist()
            ...
            if b2_name is not None and not b2_name in buses:
                ...
                buses[b2_name]["phases"] += b2_phases
                buses[b2_name]["phases"] = np.unique(
                    buses[b2_name]["phases"]).tolist()
        ...
        for name, data in buses.items():
            ...
            try:
                api_node.phases = list(
                    map(lambda x: Unicode(self.phase_mapping(x)),
                        data["phases"]))
            except:
                pass

            self._nodes[api_node.name] = api_node
    ...
    def parse_lines(self, model):
        ...
        for name, data in lines.items():
            ...
            try:
                ...
                # Do a quick check to determine if underground line
                if 'cable' in line_name or ('UG' in data):
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "Could not find the distance unit for line {name}. Using kilometers instead..."
                        .format(name=name))
                    line_unit = u"km"

            if line_unit.lower() not in [
                    "ft", "mi", "m", "km", "kft", "cm", "in"
            ]:
                line_unit = u"km"
            ...
            if phases_bus1 != phases_bus2:
                logger.warning(
                    "Phases do not match for line {name}. Bus1={b1}. Bus2={b2}".
                    format(name=name, b1=data["bus1"], b2=data["bus2"]))
            ...
            if line_name in fuses_names or "fuse" in line_name:
            ...
            # is_recloser
                ...
                if data["Switch"] == 'yes' or data["Switch"] == 'Yes':
            ...
            if line_unit is not None and Rmatrix is not None and Xmatrix is not None:
                try:
                    if (isinstance(Rmatrix, list) and len(Rmatrix) == 1 and
                            "|" in Rmatrix[0]):
                    ...
                    if (isinstance(Xmatrix, list) and len(Xmatrix) == 1 and
                            "|" in Xmatrix[0]):
            ...
            if Cmatrix is not None:
                # capacitance matrix
                try:
                    if (isinstance(Cmatrix, list) and len(Cmatrix) == 1 and
                            "|" in Cmatrix[0]):
            ...
            if line_geometry_code is not None:
                try:
                    line_geometries = dss.utils.class_to_dataframe(
                        "linegeometry")
                    this_line_geometry = line_geometries[
                        "linegeometry.{}".format(line_geometry_code)]
                except:
                    logger.warning(
                        "Could not get the geometry {line_geom} data of line {line_name}"
                        .format(line_geom=line_geometry_code, line_name=name))
            ...
            for p in range(number_of_conductors):
                ...
                if this_line_geometry is not None:
                    ...
                    # nameclass
                    try:
                        wires[p].nameclass = this_line_geometry["wires"][
                            0].split(" ")[p]
                    except:
                        pass
                    ...
                    if isinstance(this_line_geometry["units"], list):
                        ...
                        logger(
                            "Could not find the lineGeometry distance unit for line {name}."
                            .format(name=name))
                        if unit is not None:
                            logger.info(
                                "Using the line unit instead: {unit}".format(
                                    unit=unit))
                        ...
                        if "x" in this_line_geometry:
                            if isinstance(this_line_geometry["x"], list):
                                try:
                                    ...
                                    geom_x = None
                            elif isinstance(this_line_geometry["x"],
                                            (str, int, float)):
                        ...
                        if "h" in this_line_geometry:
                            if isinstance(this_line_geometry["h"], list):
                                try:
                                    ...
                                    geom_y = None
                            elif isinstance(this_line_geometry["h"],
                                            (str, int, float)):
                    ...
                    if "wires" in this_line_geometry:
                        if isinstance(this_line_geometry["wires"], list):
                            if (len(this_line_geometry["wires"]) == 1 and
                                    " " in this_line_geometry["wires"][0]):
                                this_line_wireData_code = (
                                    this_line_geometry["wires"][0].split(
                                        " ")[p].lower())
                            else:
                                try:
                            ...
                            this_line_wireData_code = None
                        if (this_line_wireData_code is None and
                                "wire" in this_line_geometry and
                                isinstance(this_line_geometry["wire"], str)):
                    ...
                    if this_line_wireData_code is not None:
                        try:
                            all_wire_data = dss.utils.class_to_dataframe(
                                "wiredata")
                            CNData = dss.utils.class_to_dataframe("CNData")
                            TSData = dss.utils.class_to_dataframe("TSData")
                            for tsname, tsvalues in TSData.items():
                                if this_line_wireData_code == tsname.split(
                                        ".")[1]:
                                    is_cable = True
                                    this_line_wireData = TSData[
                                        "TSData.{}".format(
                                            tsname.split(".")[1])]
                                    api_line.line_type = "underground"
                            for cnname, cnvalues in CNData.items():
                                if this_line_wireData_code == cnname.split(
                                        ".")[1]:
                                    is_cable = True
                            ...
                            logger.warning(
                                "Could not get the wireData {wiredata} of lineGeometry {line_geom}"
                                .format(
                    ...
                    if this_line_wireData is not None:
                        ...
                        # If not present, assume the same unit as the lineGeometry
                            ...
                            logger(
                                "Could not find the wireData radius distance unit for Wiredata {name}."
                                .format(name=this_line_wireData_code))
                            if line_geometry_unit is not None:
                                logger.info(
                                    "Using the lineGeometry unit instead: {unit}"
                                    .format(unit=line_geometry_unit))
                        ...
                        # If not present, assume the same unit as the lineGeometry
                            ...
                            logger(
                                "Could not find the wireData GMR distance unit for Wiredata {name}."
                                .format(name=this_line_wireData_code))
                            if line_geometry_unit is not None:
                                logger.info(
                                    "Using the lineGeometry unit instead: {unit}"
                        ...
                        if wire_gmr_unit is not None:
                            try:
                                wires[p].gmr = self.convert_to_meters(
                                    float(this_line_wireData["GMRac"]),
                                    wire_gmr_unit)
                            except:
                                pass
                        ...
                        # ampacity
                        try:
                            wires[p].ampacity = float(
                                this_line_wireData["normamps"])
                        except:
                            pass
                        ...
                        if (wires[p].emergency_ampacity == -1 or
                                wires[p].emergency_ampacity == 0):
                        ...
                        if api_line.length is not None:
                            ...
                            if Rac is not None:
                                ...
                                # But log this, because it might not be the case. Assume the user is responsible here....
                                    ...
                                    logger.warning(
                                        "Could not find the resistance unit for wire {wire}"
                                        .format(wire=this_line_wireData_code))
                                    if unit is not None:
                                        logger.info(
                                            "Using line length unit instead: {unit}"
                                            .format(unit=unit))
                                        Runits = unit
                                    else:
                                        Runits = None
                    ...
                    if wires[
                            p].emergency_ampacity is None and "emergamps" in data:
                        try:
                            wires[p].emergency_ampacity = float(
                                data["emergamps"])
                        except:
                        ...
                        if cndata is not None:
                            for name, data in cndata.items():
                                try:
                                    ...
                                    logger(
                                        "Could not find the GMRunits for {name}."
                                        .format(name=name))
                                if gmr_unit is not None:
                                    try:
                                        wires[
                                            p].concentric_neutral_gmr = self.convert_to_meters(
                                                float(data["GmrStrand"]),
                                                gmr_unit)
                                    except:
                                ...
                                try:
                                    ...
                                    logger(
                                        "Could not find the Runits for {name}.".
                                        format(name=name))
                                if r_unit is not None:
                                    try:
                                        wires[
                                            p].concentric_neutral_resistance = self.convert_to_meters(
                                ...
                                try:
                                    ...
                                    logger(
                                        "Could not find the Radunits for {name}."
                                        .format(name=name))
                                if rad_unit is not None:
                                    try:
                                        wires[
                                            p].concentric_neutral_diameter = self.convert_to_meters(
                                                float(data["DiaStrand"]),
                                                data["radunits"])
                                        wires[
                                            p].concentric_neutral_outside_diameter = self.convert_to_meters(
                                                float(data["DiaCable"]),
                                                data["radunits"])
                                        wires[
                                            p].insulation_thickness = self.convert_to_meters(
                                                float(data["InsLayer"]),
                                                data["radunits"])
                                    except:
                                        logger("Could not convert to radunits")
                                wires[p].concentric_neutral_nstrand = int(
                                    data["k"])

            api_line.wires = wires

            if getattr(api_line, 'is_switch', None):
                self._switches[api_line.name] = api_line
            elif getattr(api_line, 'is_fuse', None):
                self._fuses[api_line.name] = api_line
            elif getattr(api_line, 'is_recloser', None):
                self._reclosers[api_line.name] = api_line
            elif getattr(api_line, 'is_breaker', None):
                self._breakers[api_line.name] = api_line
            else:
                self._lines[api_line.name] = api_line
        ...
        for name, data in transformers.items():
            ...
            try:
                ...
                if trans_name not in self.all_object_names:
                    ...
                    logger.warning("Duplicate object Transformer {name}".format(
            ...
            try:
                api_transformer.loadloss = float(
                    data["%loadloss"])  # DiTTo in volt ampere
            except:
                pass
            ...
            if not 1 <= N_phases <= 3:
                logger.warning(
                    "Number of phases should be between 1 and 3, got {N} for transformer {name}"
                    .format(N=N_phases, name=name))

            if (N_windings >= 2 and data["conns"][0].lower() == "wye" and
                    data["conns"][1].lower() == "wye"):
                api_transformer.phase_shift = 0

            if (N_windings >= 2 and data["conns"][0].lower() == "delta" and
                    data["conns"][1].lower() == "delta"):
                api_transformer.phase_shift = 0
            ...
            for w in range(N_windings):
                ...
                try:
                    windings[w].rated_power = (float(data["kVAs"][w]) * 10**3
                                              )  # DiTTo in volt ampere
                except:
                ...
                try:
                    windings[w].emergency_power = (float(data["emerghkVA"]) *
                                                   10**3
                                                  )  # DiTTo in volt ampere
                except:
                ...
                # nominal_voltage
                try:
                    windings[w].nominal_voltage = (float(data["kVs"][w]) * 10**3
                                                  )  # DiTTo in Volts
                except:
                    pass
                ...
                for p in range(len(b1_phases)):
                    ...
                    # phase
                    try:
                        phase_windings[p].phase = self.phase_mapping(
                            b1_phases[p])
                    except:
                        pass
                    ...
                    for reg_name, reg_data in regulators.items():
                        ...
                        if ("transformer" in reg_data and
                                reg_data["transformer"].lower()
            ...
            if N_windings == 2:
                ...
                elif float(data["kVs"][1]) == max(list(map(float,
                                                           data["kVs"]))):
                    windings[0].voltage_type = 2
                    windings[1].voltage_type = 0
                    windings[2].voltage_type = 2
            ...
            self._transformers[api_transformer.name] = api_transformer
        return 1
        ...
        for name, data in regulators.items():
            ...
            try:
                ...
                if reg_name not in self.all_object_names:
                    ...
                    logger.warning("Duplicate object Regulator {name}".format(
                        name=reg_name))
            ...
            if ("transformer" in data and "Transformer.{}".format(
                    data["transformer"].lower()) in transformers):
                trans = transformers["Transformer.{}".format(
                    data["transformer"].lower())]

                # Total number of windings
                N_windings = int(trans["windings"])
                if (N_windings >= 2 and trans["conns"][0].lower() == "wye" and
                        trans["conns"][1].lower() == "wye"):
                    api_regulator.phase_shift = 0

                if (N_windings >= 2 and trans["conns"][0].lower() == "delta" and
                        trans["conns"][1].lower() == "delta"):
                    api_regulator.phase_shift = 0
                ...
                api_regulator.windings = [
                    Winding(model) for _ in range(N_windings)
                ]
                ...
                for w in range(N_windings):
                    if "conns" in trans:
                        try:
                            api_regulator.windings[w].connection_type = trans[
                                "conns"][w]
                        except:
                            pass
                ...
                for w in range(N_windings):
                    if "kVAs" in trans:
                        try:
                            api_regulator.windings[w].rated_power = (float(
                                trans["kVAs"][w]))  # DiTTo in volt ampere
                        except:
                            pass
                ...
                for w in range(N_windings):
                    if "emerghkVA" in trans:
                        try:
                            api_regulator.windings[w].emergency_power = (float(
                                trans["emerghkVA"]))  # DiTTo in volt ampere
                        except:
                            pass
                ...
                for w in range(N_windings):
                    ...
                    for p, phase in enumerate(phases):
                        # phase
                        api_regulator.windings[w].phase_windings[
                            p].phase = self.phase_mapping(phase)

                        # tap_position
                        if "taps" in trans:
                            api_regulator.windings[w].phase_windings[
                                p].tap_position = float(trans["taps"][w])

                        elif "TapNum" in data:
                            api_regulator.windings[w].phase_windings[
                                p].tap_position = float(data["TapNum"])

                        # compensator_r
                        if "R" in data:
                            try:
                                api_regulator.windings[w].phase_windings[
                                    p].compensator_r = float(data["R"])
                            except:
                                pass
                        ...
                        if "X" in data:
                            try:
                                api_regulator.windings[w].phase_windings[
                                    p].compensator_x = float(data["X"])
                            except:
                                pass

            else:
                logger.warning(
                    "Could not find the transformer data for regulator {name}".
                    format(name=name))
            ...
            if N_windings == 2:
                ...
                if float(trans["kVs"][0]) == max(list(map(float,
                                                          trans["kVs"]))):
                    api_regulator.windings[0].voltage_type = 0
                    api_regulator.windings[1].voltage_type = 2
                    api_regulator.windings[2].voltage_type = 2
                elif float(trans["kVs"][1]) == max(
                        list(map(float, trans["kVs"]))):
                    api_regulator.windings[0].voltage_type = 2
                    api_regulator.windings[1].voltage_type = 0
                    api_regulator.windings[2].voltage_type = 2
                else:
            ...
            # number of tap steps
            try:
                api_regulator.numtaps = int(trans["NumTaps"])
            except:
                pass

            # max aR
            try:
                api_regulator.maxtap = float(trans["MaxTap"])
            except:
                pass

            # min aR
            try:
                api_regulator.mintap = float(trans["MinTap"])
            except:
                pass
            ...
            if api_regulator.connected_transformer is not None:
                for trans in list(self._transformers.values()):
            ...
            self._regulators[api_regulator.name] = api_regulator
        ...
        for name, data in capacitors.items():
            ...
            try:
                ...
                if cap_name not in self.all_object_names:
                    ...
                    logger.warning("Duplicate object Capacitor {name}".format(
                        name=cap_name))
            ...
            # Nominal voltage
            try:
                api_capacitor.nominal_voltage = (float(data["kv"]) * 10**3
                                                )  # DiTTo in volts
            except:
                pass
            ...
            # Measuring element
            try:
                api_capacitor.measuring_element = cap_control[control_id][
                    "element"]
            except:
                pass
            ...
            # resistance
            try:
                api_capacitor.resistance = sum(
                    map(lambda x: float(x), data["R"]))
            except:
                pass

            # reactance
            try:
                api_capacitor.reactance = sum(
                    map(lambda x: float(x), data["XL"]))
            except:
                pass
            ...
            if control_id is not None:
                try:
                    api_capacitor.pt_ratio = float(
                        cap_control[control_id]["PTratio"])
                except:
                    pass
            ...
            if control_id is not None:
                try:
                    api_capacitor.ct_ratio = float(
                        cap_control[control_id]["CTratio"])
                except:
                    pass
            ...
            if phases is not None and N_phases is not None:
                ...
                for p, pha in enumerate(phases):
                    ...
                    # phase
                    if (api_capacitor.pt_phase is not None and
                            api_capacitor.pt_phase == self.phase_mapping(pha)):
                    ...
                    # var
                    list_data = data["kvar"]
                    if isinstance(list_data,
                                  list) and len(list_data) == len(phases):
                        try:
                            phase_capacitors[p].var = (float(list_data[p]) *
                                                       10**3)  # DiTTo in var
                        except:
                            pass
                    elif isinstance(list_data, list):
                        try:
                            phase_capacitors[p].var = (float(list_data[0]) /
                                                       float(N_phases) * 10**3
                                                      )  # DiTTo in var
                        except:
                            pass
                        ...
                        try:
                            phase_capacitors[p].var = (float(list_data) /
                                                       float(N_phases) * 10**3
                                                      )  # DiTTo in var
                        except:
                            pass
            ...
            self._capacitors[api_capacitor.name] = api_capacitor
    ...
    def parse_loads(self, model):
        ...
        for name, data in loads.items():
            ...
            # nominal voltage
            try:
                api_load.nominal_voltage = float(
                    data["kV"]) * 10**3  # DiTTo in volts
            except:
                pass
            ...
            try:
                conn = data["conn"]
                if conn.lower() == "wye" or conn.lower() == 'w':
                    api_load.connection_type = "Y"
                elif conn.lower() == "delta" or conn.lower() == 'd':
            ...
            if N_phases != len(phases):
                if N_phases is None and phases is None:
                    logger.warning(
                        "No Phase information for load {name}".format(
                            name=name))
            ...
            # Get the kVar if present
            try:
                kvar = float(data["kvar"])
            except:
                kvar = None
            ...
            for i, p in enumerate(phases):
                ...
                # Calculate phase voltage for Constant Z and I use
                _Vnom = api_load.nominal_voltage

                if _phase_loads[i].phase == 'A':
                    _deg = 0
                elif _phase_loads[i].phase == 'B':
                    _deg = -120
                else:
                    _deg = 120
                _Vn = _Vnom * complex(np.cos(np.deg2rad(_deg)),
                                      np.sin(np.deg2rad(_deg)))
                _PQ = complex(_phase_loads[i].p, _phase_loads[i].q)

                # Constant Z model (model==2)
                if _model == 2:
                    _phase_loads[i].z = np.abs(_Vn)**2 / np.conj(_PQ)
                    _phase_loads[i].p = 0
                    _phase_loads[i].q = 0
                # Exponential model (model==4)
                elif _model == 4:
                    _phase_loads[i].cvrwatts = float(data["CVRwatts"])
                    _phase_loads[i].cvrvars = float(data["CVRvars"])
                #  Constant I model (model==5)
                elif _model == 5:
                    _phase_loads[i].i = np.conj(_PQ / _Vn)
                    _phase_loads[i].p = 0
                    _phase_loads[i].q = 0
                # ZIPV model (model==8)
                elif _model == 8:
                    ...
                    try:
                        ZIPV = list(
                            map(lambda x: float(x), data["ZIPV"].split()))
                    except:
                        ZIPV = None
                    ...
                    if ZIPV is not None:
                        ...
                        if not np.allclose(sum(ZIPV[:3]),
                                           1.0) or not np.allclose(
                                               sum(ZIPV[3:-1]), 1.0):
                            logger.warning(
                                "ZIPV coefficients for load {name} do not sum properly"
                                .format(name=name))
            ...
            self._loads[api_load.name] = api_load
    ...
    def parse_storage(self, model):
        ...
        for name, data in storages.items():
            ...
            try:
                api_storage.discharging_efficiency = float(
                    data["%EffDischarge"])
            except:
                pass
    ...
    @timeit
    def parse_reactors(self, model):
        reactors = dss.utils.class_to_dataframe("reactor")

        for name, data in reactors.items():

            #Skip reactor object if disabled
            if not data["enabled"]:
                continue
            ...
            try:
                if "." in name:
                    reactor_name = "reactor_" + name.split("eactor.")[1].lower()
                else:
                    reactor_name = name
            ...
            try:
                api_reactor.nominal_voltage = (float(data["kv"]) * 10**3
                                              )  # DiTTo in volts
            except:
                pass
            ...
            try:
                if "." in data["bus1"]:
                    temp = data["bus1"].split(".")
                    from_name = temp[0].lower()
                    phases = temp[1:]
                else:
                    from_name = data["bus1"].lower()
                    phases = [1, 2, 3]

                if "." in data["bus2"]:
                    temp = data["bus2"].split(".")
                    to_name = temp[0].lower()
                else:
                    to_name = data["bus2"].lower()

                api_reactor.from_element = from_name
                api_reactor.to_element = to_name
            except:
                from_name = None
                to_name = None
                phases = None
            ...
            # connection_type
            try:
                if data["conn"].lower() == "wye" or data["conn"].lower(
                ) == "ln":
                    api_reactor.connection_type = "Y"
                elif data["conn"].lower() == "delta" or data["conn"].lower(
                ) == "ll":
            ...
            # faultrate
            try:
                api_reactor.faultrate = data["faultrate"]
            except:
                pass
            ...
            if phases is not None and N_phases is not None:
                ...
                for p, pha in enumerate(phases):
                    ...
                    phase_reactors[p].phase = self.phase_mapping(pha)

                    phase_reactors[p].ampacity = float(data["normamps"])
                    phase_reactors[p].emergency_ampacity = float(
                        data["emergamps"])

                    res = data["R"]
                    reac = data["X"]

                    if isinstance(res, list) and len(res) == len(phases):
                        try:
                            phase_reactors[p].resistance = (float(res[p]))
                        except:
                            pass
                    elif isinstance(res, list):
                        try:
                            phase_reactors[p].resistance = (float(res[0]))
                        except:
                            pass
                    else:
                        try:
                            phase_reactors[p].resistance = (float(res))
                        except:
                            pass

                    if isinstance(reac, list) and len(reac) == len(phases):
                        try:
                            phase_reactors[p].reactance = (float(reac[p]))
                        except:
                            pass
                    elif isinstance(reac, list):
                        try:
                            phase_reactors[p].reactance = (float(reac[0]))
                        except:
                            pass
                    else:
                        try:
                            phase_reactors[p].reactance = (float(reac))
                        except:
                            pass

                api_reactor.phase_reactors = phase_reactors

            self._reactors[api_reactor.name] = api_reactor

            return 1

    @timeit
    def parse_photovoltaics(self, model):
        """Photovoltaic parse.
        ...
        photovoltaics = dss.utils.class_to_dataframe("PVSystem")

        for name, data in photovoltaics.items():

            # Skip Photovoltaic object if disabled
            if not data["enabled"]:
                continue

            api_photovoltaic = Photovoltaic(model)
            api_photovoltaic.feeder_name = self.source_name

            #Name
            try:
                photovoltaic_name = "pv_" + name.replace("PVSystem.",
                                                         "").lower()
                if photovoltaic_name not in self.all_object_names:
                    self.all_object_names.append(photovoltaic_name)
                else:
                    logger.warning(
                        "Duplicate object Photovoltaic {name}".format(
                            name=photovoltaic_name))
                api_photovoltaic.name = photovoltaic_name
            except:
                pass

            # nominal voltage
            try:
                api_photovoltaic.nominal_voltage = float(data["kv"]) * 10**3
            except:
                pass
            ...
            # vmin
            try:
                api_photovoltaic.v_min_pu = float(data["Vminpu"])
            except:
                pass

            # vmax
            try:
                api_photovoltaic.v_max_pu = float(data["Vmaxpu"])
            except:
                pass
            ...
            api_photovoltaic.phases = list(
                map(lambda x: Unicode(self.phase_mapping(x)), b1_phases))

            # Connecting_element
            api_photovoltaic.connecting_element = b1_name.lower()

            # rated_power
            try:
                api_photovoltaic.rated_power = float(data["kVA"]) * 10**3
            except:
                pass

            # reactive_rating
            try:
                api_photovoltaic.reactive_rating = float(data["kvar"]) * 10**3
            except:
                pass

            # active_rating
            try:
                api_photovoltaic.active_rating = float(data["kVA"]) * float(
                    data["pf"]) * 10**3
            except:
                pass
            ...
            # power factor
            try:
                api_photovoltaic.power_factor = data["pf"]
            except:
                pass

            # cutout_percent
            try:
                api_photovoltaic.cutout_percent = float(data["%Cutout"])
            except:
                pass

            # cutin_percent
            try:
                api_photovoltaic.cutin_percent = float(data["%Cutin"])
            except:
                pass

            # irradiance
            try:
                api_photovoltaic.irradiance = float(data["irradiance"])
            except:
                pass

            # pv_power_max
            try:
                api_photovoltaic.pv_power_max = float(data["Pmpp"]) * 10**3
            except:
                pass

            self._photovoltaics[api_photovoltaic.name] = api_photovoltaic
