...
class reader:
    ...
    def parse(self, model, input_file):
        ...
        for row_idx in range(dataframe.shape[0]):
            ...
            all_objs = {
            }  # Create a new set of objects for each row of the csv file
            for column in dataframe.columns:
                ...
                if dataframe.iloc[row_idx][column] == "" or (
                        isinstance(dataframe.iloc[row_idx][column], float) and
                        math.isnan(dataframe.iloc[row_idx][column])
                ...
                parent_obj = all_objs[layers[
                    0]]  # We recursively update the parent object for each subelement
                for i in range(
                        1,
                        len(layers) -
                        1):  # i.e. all the object lists that need to be called
                    layer_name = layers[i].split("[")[
                        0]  # subclass element name
                    layer_number = int(layers[i].split("[")[1].strip(
                        "]"))  # subclass element index.
                    klass = parent_obj.traits(
                    )[layer_name]._trait.klass  # find the objects that the list contains
                    klassname = (
                ...
                if layers[-1] == "name":
                    setattr(
                        parent_obj, layers[-1],
                        dataframe.iloc[row_idx][column].lower(
