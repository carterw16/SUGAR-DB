...
class Reader(AbstractReader):
    ...
    def get_file_content(self):
        ...
        try:
            ...
            raise ValueError("Unable to open project from {name}".format(
                name=self.windmil_folder))
        return
    ...
    def parse_lines(self, model):
        ...
        self.nxGraph.graph["Construction Code"]
    ...
    def create_device(self, model, node1, node2, Devices):
        ...
        device.feeder_name = (
            "" if not isinstance(self.nxGraph[node1][node2]["feeder"], str) else
            ...
            if self.nxGraph[node1][node2]["type"] == "underground":
                ...
                if len(cond_data):
                    ...
                    phase_wire.concentric_neutral_diameter = self.convert_to_meters(
                        float(cond_data["OD of Cable Including Neutral In Fee"].
                              iloc[0]),
                        "ft",
                    )
                    phase_wire.ampacity = float(
                    ...
                    phase_wire.resistance = self.convert_from_meters(
                        float(cond_data["Phase Conductor Resistance Ohms/Mile"].
                              iloc[0]),
                        "mi",
                    )
    ...
    def parse_feeder_metadata(self, model):
        ...
        for node1, node2 in PowerSources:
            ...
            Source.nominal_voltage = (1.732 *
                                      float(self.nxGraph[node1][node2]["kv"]) *
                                      1000)
            ...
            Source.connection_type = ("Y" if self.nxGraph[node1][node2]["conn"]
                                      == "W" else "D")
    ...
    def CreateRegXfmr(self, model, node1, node2, reg_data, reg_type_data):
        ...
        for i in range(2):
            ...
            wdg.rated_power = (float(reg_data["kv"][0]) *
                               float(reg_type_data.iloc[-1]["Ampacity"]) * 1000)
            wdg.voltage_type = 0 if i == 0 else 2
