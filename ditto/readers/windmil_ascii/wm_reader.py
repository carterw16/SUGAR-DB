...
from bokeh.models import (BoxSelectTool, BoxZoomTool, ColumnDataSource, PanTool,
...
class wm2graph:
        ...
        try:
            ...
            for file in file_list:
                if file.lower().endswith('.asm'):
                    self.files['Assemblies'] = os.path.join(
                        project_folder, file)
                elif file.lower().endswith('.cld'):
                    self.files['Archieved Load'] = os.path.join(
                        project_folder, file)
                elif file.lower().endswith('.lcp'):
                    self.files['Load Control Points'] = os.path.join(
                        project_folder, file)
                elif file.lower().endswith('.sld'):
                    self.files['Billing Load Data'] = os.path.join(
                        project_folder, file)
                elif file.lower().endswith('.csv'):
                    self.files['External Tables'] = os.path.join(
                        project_folder, file)
                elif file.lower().endswith('.seq'):
                    self.files['Equipment Data'] = os.path.join(
                        project_folder, file)
                elif file.lower().endswith('.std'):
                    self.files['Circuit Elements'] = os.path.join(
                        project_folder, file)
                    ...
                    self.files['Map Points'] = os.path.join(
                        project_folder, file)
            if len(self.files) == 0:
                raise ValueError(
            ...
            raise ValueError(
                'Folder path "{}" does not exist '.format(project_folder))

        no_header = [
            'Map Points', 'Archieved Load', 'Billing Load Data',
            'Equipment Data', 'Circuit Elements'
        ]
    ...
    def create_plot(self, updateCoordinates):
        ...
        if updateCoordinates:
            ...
            for edge in edges:
                node1, node2 = edge
                if 'x' in self.nxGraph.node[node1] and 'x' in self.nxGraph.node[
                        node2]:
                ...
                for node in [node1, node2]:
                    if 'loads' in self.nxGraph.node[node]:
                        for load_name, load_properties in self.nxGraph.node[
                                node]['loads'].items():
                            if 'x' in self.nxGraph.node[
                                    node] and self.nxGraph.node[node]:
        ...
        plot.multi_line(xs="Xs",
                        ...
                        line_width=2,
                        legend='Edges',
                        line_color='#00008B')  # , line_color=ColorBy
        # plot loads
        ...
        plot.hex(x=Xs,
                 ...
                 line_color='black')
        plot.multi_line(xs=load_lines_xs,
                        ys=load_lines_ys,
                        line_width=2,
                        legend='Drop lines',
                        line_color='black')
    ...
    def create_generators(self):
        ...
        for r, generator in generators.iterrows():
            from_node = 'node_' + self.fix_string(
                generator['Parent Element Name'])
            ...
            if from_node in self.nxGraph.nodes():
                ...
                self.nxGraph.node[from_node]['capacitors'][self.fix_string(
                    generator['Element Name'])] = {
                        ...
                        'Vfix':
                            generator['Voltage to Hold'],
                        'kvRated':
                            generator['Rated Voltage'],
                        'load location':
                            generator['Load Location'],
                        'load mix':
                            generator['Load Mix Description'],
                        'load zone':
                            generator['Load Zone Description'],
                        'load growth':
                            generator['Load Growth'],
                        'kw': [
                        ...
                        'kwOut':
                            generator['kW Out'],
                        'kwOut max':
                            generator['Maximum kW Out'],
                        'kvar max lead':
                            generator['Maximum kvar Lead Output'],
                        'kvar max lag':
                            generator['Maximum kvar Lagg Output'],
                        'conn':
                            generator_conn[int(
                                generator['Wye or Delta Connection'])],
                        'failure rate':
    ...
    def create_capacitors(self):
        ...
        for r, capacitor in capacitors.iterrows():
            print(capacitor)
            from_node = 'node_' + self.fix_string(
                capacitor['Parent Element Name'])
            #to_node = 'node_' + self.fix_string(load['Element Name'])
            if from_node in self.nxGraph.nodes():
                if 'capacitors' not in self.nxGraph.node[from_node]:
                    self.nxGraph.node[from_node]['capacitors'] = {}
                self.nxGraph.node[from_node]['capacitors'][self.fix_string(
                    capacitor['Element Name'])] = {
                        'name':
                            self.fix_string(capacitor['Element Name']),
                        'phases':
                            capacitor['Phase Configuration'],
                        'x':
                            capacitor['X Coordinate'],
                        'y':
                            capacitor['Y Coordinate'],
                        'On Setting':
                            capacitor['Switch On Setting'],
                        'Off Setting':
                            capacitor['Switch Off Setting'],
                        'state':
                            capacitor_state[int(capacitor['Switch Status Code'])
                                           ],
                        'reg mode':
                            capacitor_control_type[int(
                        ...
                        'Ctrl Element':
                            capacitor['Control Element'],
                        'conn':
                            capacitor_conn[capacitor['Connection']],
                        'kvarRated':
                            capacitor['Unit Size kvar'],
                        'control phase':
                            capacitor['Control Phase'],
                        'failure rate':
                            capacitor['Failure Rate'],
                        'repair Time':
                            capacitor['Repair Time'],
                        'bypass Time':
                            capacitor['Bypass Time'],
                    }
        return
    ...
    def create_loads(self):
        ...
        for r, load in service_locations.iterrows():
            ...
            'node_' + self.fix_string(load['Element Name'])
            if from_node in self.nxGraph.nodes():
                if 'loads' not in self.nxGraph.node[from_node]:
                    self.nxGraph.node[from_node]['loads'] = {}
                self.nxGraph.node[from_node]['loads'][self.fix_string(
                    load['Element Name']
                )] = {
                    ...
                    'kw': [
                        ...
                        load['Allocated kW (Ph B)'], load['Allocated kW (Ph C)']
                    ],
                    'kvar': [
        ...
        for r, node in nodes.iterrows():
            ...
            reg_dict = {
                ...
                'kw': [
                    ...
                    node['Allocated kW, Phase B'], node['Allocated kW, Phase C']
                ],
                'kvar': [
                ...
                'parents': [
                    node['A Phase Parent'], node['B Phase Parent'],
                    node['C Phase Parent']
                ],
                'isMandatory': node['Node Is Mandatory'],
                'enabled': [
                    node['Phase A Energized'], node['Phase B Energized'],
    ...
    def create_substation(self):
        ...
        for r, substation in substations.iterrows():
            from_node = 'node_' + self.fix_string(
                substation['Parent Element Name'])
        ...
        for r, reg in regs.iterrows():
            ...
            reg_dict = {
                ...
                'LDCr': [
                    ...
                    reg['LDC R Setting, Phase B'], reg['LDC R Setting, Phase C']
                ],
                'LDCx': [
                    reg['LDC X Setting, Phase A'],
                    reg['LDC X Setting, Phase B'], reg['LDC X Setting, Phase C']
                ],
                'fhhp': [
                ...
                'enabled': [
                    reg['Phase A Energized'], reg['Phase B Energized'],
    ...
    def create_transformers(self):
        ...
        for r, xfmr in xfmrs.iterrows():
            ...
            fuse_dict = {
                ...
                'kva': [
                    xfmr['Tran kVA A'], xfmr['Tran kVA B'], xfmr['Tran kVA C']
                ],
                'regulators': [
                    xfmr['Xfmr Cond Desc. Ph A'], xfmr['Xfmr Cond Desc. Ph B'],
    ...
    def create_devices(self):
        ...
        for r, fuse in fuses.iterrows():
            ...
            fuse_dict = {
                'name':
                    self.fix_string(fuse['Element Name']),
                'class':
                    'device',
                'phases':
                    fuse['Phase Configuration'],
                'Control point':
                    fuse['Load Allocation Control Point'],
                'equipment': [
                    fuse['Description, Ph A'], fuse['Description, Ph B'],
                    fuse['Description, Ph C']
                ],
                'isClosed': [
                    fuse['Is Closed, Phase A'], fuse['Is Closed, Phase B'],
                ...
                'open Time':
                    fuse['Open Time'],
                'Operation':
                    fault_coord_type[int(fuse['Fuse Coordination Method'])],
                'enabled': [
                    fuse['Phase A Energized'], fuse['Phase B Energized'],
                    fuse['Phase C Energized']
                ],
                'GID':
                    fuse['GUID'],
                'substation':
                    fuse['Substation Name'],
                'feeder':
                    fuse['Feeder Name'],
                'mGID':
                    fuse['mGUID'],
    ...
    def create_switches(self):
        ...
        for r, switch in switches.iterrows():
            ...
            switch_dict = {
                ...
                'enabled': [
                    switch['Phase A Energized'], switch['Phase B Energized'],
    ...
    def create_lines(self):
        ...
        line_tables = [
            self.library['Overhead Line'], self.library['Underground Line']
        ]
        for line_id, line_database in enumerate(line_tables):
            ...
            for r, line in line_database.iterrows():
                ...
                line_properties = {
                    ...
                    'conductors': [
                        line['Conductor Phase A'], line['Conductor Phase B'],
                        line['Conductor Phase C'], line['Conductor neutral']
                    ],
                    'enabled': [
                        line['Phase A Energized'], line['Phase B Energized'],
