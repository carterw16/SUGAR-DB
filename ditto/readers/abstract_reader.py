...
class AbstractReader(object):
    ...
    def convert_to_meters(self, quantity, unit, **kwargs):
        ...
        **Ratios:**
        ...
        if not isinstance(unit, string_types):
            self.logger.warning(
                "convert_to_meters() expects a unit in string format")
            return None
    ...
    def convert_from_meters(self, quantity, unit, **kwargs):
        ...
        **Ratios:**
        ...
        if not isinstance(unit, string_types):
            self.logger.warning(
                "convert_from_meters() expects a unit in string format")
            return None
    ...
    def distance(self, p1, p2):
        ...
        **Formula:**
        ...
        N_dim = len(p1)
        if len(p2) != N_dim:
            raise ValueError("Dimension mismatch between {p1} and {p2}.".format(
                p1=p1, p2=p2))
    ...
    def kron_reduction(self, primitive_impedance_matrix, neutrals=None):
        ...
        zin = [[None for i in range(dim_neutrals)] for j in range(dim_phase)
              ]  # z[i][n]
        znj = [[None for i in range(dim_phase)] for j in range(dim_neutrals)
              ]  # z[n][j]
    ...
    def carson_equation_self(self, ri, GMRi):
        ...
        if ri is None:
            raise ValueError(
                "Resistance is None. Cannot compute Carson's equation.")
        if GMRi is None:
    ...
    def carson_equation(self, Dij):
        ...
        if Dij == 0:
            raise ValueError(
                "Distance Dij is zero. Cannot compute Carson's equation.")
    ...
    def get_sequence_impedances(self, dist_matrix, GMR_list, r_list):
        ...
        prim = self.get_primitive_impedance_matrix(dist_matrix, GMR_list,
                                                   r_list)
    ...
    def get_phase_impedances(self,
        ...
        impedance_matrix = [
            [None for i in range(nphases)] for j in range(nphases)
        ]
        ...
        distance_matrix_feet = [
            [None for i in range(nphases)] for j in range(nphases)
        ]
        if nphases != len(distance_matrix):
            raise ValueError(
                "Distance matrix must be same size as number of wires")
        ...
        if overhead:
            ...
            for i in range(nphases):
                if wire_list[i].gmr is None:
                    ...
                    self.logger.warning("Warning - using default gmr of " +
                                        str(gmr))
                else:
                ...
                rs.append(
                    self.convert_from_meters(resistance_meters,
                                             "ft",
                                             inverse=True))
            ...
            for i in range(len(impedance_matrix)):
                for j in range(len(impedance_matrix)):
                    impedance_matrix[i][j] = self.convert_to_meters(
                        impedance_matrix_imperial[i][j], "mi",
                        inverse=True)  # works for complex
        else:
            ...
            for i in range(nphases):
                if (
                    ...
                    concentric_neutral_gmr_meters = wire_list[
                        i].concentric_neutral_gmr
                ...
                if wire_list[i].concentric_neutral_nstrand is None:
                    ...
                    concentric_neutral_nstrand = wire_list[
                        i].concentric_neutral_nstrand

                if wire_list[i].concentric_neutral_diameter is None:
                    concentric_neutral_diameter_meters = (
                        1 / 1000.0)  # 1 mm strand diameter
                    self.logger.warning(
                ...
                if wire_list[i].concentric_neutral_outside_diameter is None:
                    ...
                    concentric_neutral_outside_diameter_meters = wire_list[
                        i].concentric_neutral_outside_diameter
                ...
                if wire_list[i].concentric_neutral_resistance is None:
                    ...
                    concentric_neutral_resistance_meters = wire_list[
                        i].concentric_neutral_resistance
                ...
                if wire_list[i].gmr is None:
                    gmr_meters = (13.26 / 2.0 / 1000.0 * 0.7788
                ...
                radius_meters = (concentric_neutral_outside_diameter_meters -
                                 concentric_neutral_diameter_meters) / 2.0
                ...
                gmr_neutral = (concentric_neutral_gmr *
                ...
                r_neutral = self.convert_from_meters(
                    concentric_neutral_resistance_meters, "mi",
                    inverse=True) / float(concentric_neutral_nstrand)
                gmrs.append(self.convert_from_meters(gmr_meters, "ft"))
                rs.append(
                    self.convert_from_meters(resistance_meters,
                                             "ft",
            ...
            for i in range(nphases):
                for j in range(nphases):
                    ...
                    distance_matrix_cn[i + nphases][
                        j + nphases] = distance_matrix_feet[i][j]
                    ...
                    # As per Example 4.2 of Kersting. phase-neutral = phase-phase distance for different cables
                        ...
                        distance_matrix_cn[
                            i + nphases][j] = distance_matrix_feet[i][j]
                        distance_matrix_cn[i][
                            j + nphases] = distance_matrix_feet[i][j]
            gmrs_cn = []
            rs_cn = []
            for i in range(nphases):
    ...
    def parse(self, model, **kwargs):
        ...
        if hasattr(self, "DSS_file_names"):
            if (self.DSS_file_names["default_values_file"] or
                    self.DSS_file_names["remove_opendss_default_values_flag"] is
                    True):
                if self.verbose:
    ...
    def parse_default_values(self, model):
        ...
        for obj in model.models:
            self.set_default_values(
                obj, "faultrate",
                parsed_values.get("Line", None).get("faultrate", None))
            self.set_default_values(
                obj,
                "impedance_matrix",
            ...
            self.set_default_values(
                obj, "ampacity",
                parsed_values.get("Wire", {}).get("ampacity", None))
            self.set_default_values(
                obj,
                "emergency_ampacity",
            ...
            if type(obj).__name__ == "Capacitor":
                self.set_default_values(
                    obj,
                    "connection_type",
                    parsed_values.get("Capacitor",
                                      {}).get("connection_type", None),
                )
                self.set_default_values(
                    obj, "delay",
                    parsed_values.get("Capacitor", {}).get("delay", None))
                self.set_default_values(
                    obj,
                    "pt_ratio",
            ...
            self.set_default_values(
                obj, "low",
                parsed_values.get("Capacitor", {}).get("low", None))
            self.set_default_values(
                obj, "high",
                parsed_values.get("Capacitor", {}).get("high", None))
            self.set_default_values(
                obj,
                "ct_ratio",
            ...
            if type(obj).__name__ == "Regulator":
                self.set_default_values(
                    obj,
                    "connection_type",
                    parsed_values.get("Regulator",
                                      {}).get("connection_type", None),
                )
                self.set_default_values(
                    obj, "delay",
                    parsed_values.get("Regulator", {}).get("delay", None))
                self.set_default_values(
                    obj,
                    "pt_ratio",
            ...
            self.set_default_values(
                obj, "ct_prim",
                parsed_values.get("Regulator", {}).get("ct_prim", None))
            self.set_default_values(
                obj,
                "highstep",
            ...
            self.set_default_values(
                obj, "vmin",
                parsed_values.get("Load", {}).get("vmin", None))
            self.set_default_values(
                obj, "vmax",
                parsed_values.get("Load", {}).get("vmax", None))
