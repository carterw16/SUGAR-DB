...
class Reader(AbstractReader):
    ...
    **Default header mapping:**
    ...
    def update_header_mapping(self, update):
        ...
        **Usage:**
        ...
        if not isinstance(update, dict):
            raise ValueError(
                "update_header_mapping expects a dictionary. A {type} instance was provided"
                .format(type(update)))
    ...
    def get_file_content(self, filename):
        ...
        # Shortcut mapping
        if filename == "network":
            filename = os.path.join(self.data_folder_path,
                                    self.network_filename)
        elif filename == "equipment":
            filename = os.path.join(self.data_folder_path,
                                    self.equipment_filename)
        elif filename == "load":
        ...
        **Mapping:**
        ...
        if not isinstance(value1, float):
            try:
                ...
                raise ValueError(
                    "Value1={} could not be converted to float in load_value_type_mapping."
                    .format(value1))
        ...
        if not isinstance(value2, float):
            try:
                ...
                raise ValueError(
                    "Value2={} could not be converted to float in load_value_type_mapping."
                    .format(value2))
        ...
        if isinstance(load_type, string_types):
            ...
            if load_type == "3" or load_type.lower() == "amp_pf":
                raise NotImplementedError(
                    "AMP_PF load type not implemented yet.")
            ...
            raise ValueError(
                "load_value_type_mapping expects a string or integer for load_type. {} was provided."
                .format(type(load_type)))
    ...
    def capacitors_connection_mapping(self, conn):
        ...
        **Mapping:**
        ...
        if not isinstance(conn, (string_types, int)):
            raise ValueError(
                "capacitors_connection_mapping only accepts int or string. {} was provided."
                .format(type(conn)))
    ...
    def connection_configuration_mapping(self, value):
        ...
        **Mapping:**
        ...
        if isinstance(value, int):
            ...
            if (value == "0" or value.lower() == "yg" or value == "1" or
                    value.lower() == "y"):
                return "Y"
            if (value == "2" or value.lower() == "delta" or value == "3" or
                    value.lower() == "open delta" or value == "4" or
                    value.lower() == "closed delta"):
                return "D"
            if value == "5" or value.lower() == "zg":
                return "Z"
            if (value == "6" or value.lower() == "ct" or value == "7" or
                    value.lower() == "dg"):
                raise NotImplementedError(
                    "Connection {} not implemented.".format(value))
            ...
            raise ValueError(
                "connection_configuration_mapping expects an integer or a string. {} was provided."
                .format(type(value)))
    ...
    def transformer_connection_configuration_mapping(self, value, winding):
        ...
        **Mapping:**
        ...
        if winding not in [0, 1]:
            raise ValueError(
                "transformer_connection_configuration_mapping expects an integer 0 or 1 for winding arg. {} was provided."
                .format(winding))
        ...
        if isinstance(value, int):
            ...
            raise ValueError(
                "transformer_connection_configuration_mapping expects an integer or a string. {} was provided."
                .format(type(value)))
    ...
    def check_object_in_line(self, line, obj):
        ...
        if not isinstance(line, string_types):
            raise ValueError(
                "check_object_in_line expects a string for both line and object. A {type} instance was provided for line."
                .format(type=type(line)))

        if not isinstance(obj, string_types):
            raise ValueError(
                "check_object_in_line expects a string for both line and object. A {type} instance was provided for object."
                .format(type=type(obj)))

        if not obj in self.header_mapping:
            raise ValueError(
                "{obj} is not a valid object name for the object<->header mapping.{mapp}"
                .format(obj=obj, mapp=self.header_mapping))
                ...
                if "=" not in next_line.lower():
                    ...
                    try:
                        ...
                        if isinstance(additional_attributes, list):
                            additional_attributes = np.array(
                                additional_attributes)
                        ...
                        for idx, arg in enumerate(arg_list):
                            temp = np.argwhere(
                                arg == additional_attributes).flatten()
                            if len(temp) == 1:
                                idx2 = temp[0]
        ...
        for line in self.content:
            if "cyme_version" in line.lower():
                ...
                if cyme_version is not None:
                    logger.info(
                        "---| Cyme_version={v} |---".format(v=cyme_version))
                    if "." in cyme_version:
                        try:
    ...
    def parse_subnetwork_connections(self, model):
        ...
        for key in self.subnetwork_connections:
            model[self.subnetwork_connections[key]
                  ["nodeid"]].is_substation_connection = True
    ...
    def parse_head_nodes(self, model):
        ...
        for line in self.content:
            headnodes.update(
                self.parser_helper(line, ["headnodes"], ["nodeid", "networkid"],
                                   mapp))
    ...
    def parse_sources(self, model):
        ...
        for line in self.content:
            subs.update(
                self.parser_helper(line, ["substation"],
                                   ["id", "mva", "kvll", "conn"], mapp_sub))
            ...
            for sid, source_equivalent_data in source_equivalents.items():
                ...
                for k, v in self.section_phase_mapping.items():
                    if v["fromnodeid"] == source_equivalent_data["nodeid"]:
                        ...
                        v["tonodeid"]
                        phases = list(v["phase"])
                    if (v["tonodeid"] == source_equivalent_data["nodeid"]
                ...
                if sid in subs:
                    ...
                    try:
                        if "desiredvoltage" in sdata:
                            ...
                            api_source.nominal_voltage = (
                                float(source_equivalent_data["voltage"]) *
                                10**3)
                    except:
                        pass
                    ...
                    api_source.positive_sequence_impedance = complex(
                        float(
                            source_equivalent_data["positivesequenceresistance"]
                        ),
                        float(
                            source_equivalent_data["positivesequencereactance"]
                        ),
                    )
                    # except:
                    # pass
    ...
    def parse_nodes(self, model):
        ...
        for line in self.content:
            nodes.update(
                self.parser_helper(
                    line, ["node"],
                    ["nodeid", "coordx", "coordy", "ratedvoltage"], mapp,
                    **kwargs))
        self.get_file_content("network")
        for line in self.content:
            node_connectors.update(
                self.parser_helper(line, ["node_connector"],
                                   ["nodeid", "coordx", "coordy"], mapp))
    ...
    def configure_wire(
        ...
        if (is_network_protector or is_fuse or is_sectionalizer or is_breaker or
                is_recloser):
            try:
        ...
        try:
            api_wire.emergency_ampacity = float(
                conductor_data["withstandrating"])
        except:
            pass
    ...
    def parse_sections(self, model):
        ...
        **Data structures:**
        ...
        .. warning:: This should be called prior to any other parse because the other parsers rely on these 3 data structures.
        """
        ...
        for line in self.content:
            ...
            if "[SECTION]" in line:
                ...
                while len(line) > 2 and (line[0] != "[" or line[0] != " " or
                                         line[0] != "\n" or line[0] != "\t\n"):
                    ...
                    elif ("format_feeder" in line.lower() or
                          "format_substation" in line.lower() or
                          "format_generalnetwork" in line.lower()):
                        format_feeder = list(
                            map(
                                lambda x: x.strip(),
                                map(lambda x: x.lower(),
                    ...
                    # If we have a new feeder declaration
                            ...
                            line[:7].lower() == "feeder=" or
                            line[:11].lower() == "substation=" or
                            line[:11].lower() == "substation=" or
                            line[:15].lower() == "generalnetwork="):
                        if (line[:7].lower() == "feeder=" or
                                line[:15].lower() == "generalnetwork="):
                        ...
                        if len(feeder_data) != len(format_feeder):
                            raise ValueError(
                                "Feeder/substation data length {a} does not match feeder format length {b}."
                                .format(a=len(feeder_data),
                                        b=len(format_feeder)))
                        ...
                        _netID = feeder_data[format_feeder.index(
                            "networkid")].lower()
                    ...
                    # Otherwise, we should have a new section...
                        ...
                        if len(section_data) != len(format_section):
                            raise ValueError(
                                "Section data length {a} does not match section format length {b}."
                                .format(a=len(section_data),
                                        b=len(format_section)))

                        # Grab the sectionid
                        _sectionID = section_data[format_section.index(
                            "sectionid")].lower()
    ...
    def parse_lines(self, model):
        ...
        for line in self.content:
            ...
            self.settings = self.update_dict(
                ...
                self.parser_helper(
                    ...
                    [
                        "sectionid", "coordx", "coordy", "linecableid",
                        "length", "amps"
                    ],
                    mapp_underground,
                    {"type": "underground"},
        ...
        for line in self.content:
            ...
            self.switches.update(
                self.parser_helper(line, ["switch"], ["id", "amps", "kvll"],
                                   mapp_switch_eq))
        ...
        for sectionID, settings in self.settings.items():
            ...
            # TODO: CLEAN THIS...
            if ("load" in settings["fromnodeid"].lower() or
                    "load" in settings["tonodeid"].lower()):
                continue
            ...
            try:
                new_line["from_element"] = self.section_phase_mapping[
            ...
            if "type" in settings:
                ...
                # Switch
                    ...
                    try:
                        new_line["nominal_voltage"] = float(
                            switch_data["kvll"]) * 1000
                    except:
                        pass
                ...
                # Sectionalizer
                    ...
                    # Get the sectionalizer equipment data
                    if "eqid" in settings and settings[
                            "eqid"] in self.sectionalizers:
                        sectionalizer_data = self.sectionalizers[
                            settings["eqid"]]
                    else:
                        sectionalizer_data = {}
                ...
                # Network Protectors
                    ...
                    if ("eqid" in settings and
                            settings["eqid"] in self.network_protectors):
                    ...
                    # Pass the nameclass to the wires
                    if "nameclass" in new_line:
                        network_protector_data["nameclass"] = new_line[
                            "nameclass"]
            ...
            if "linecableid" in settings:
                ...
                if settings["linecableid"] in self.concentric_neutral_cable:
                    # Cache the line data
                    line_data = self.concentric_neutral_cable[
                        settings["linecableid"]]
            ...
            #
                ...
                if settings["devicenumber"] in self.concentric_neutral_cable:
                    ...
                    line_data["type"] = "balanced_line"
                elif ("condid_a" in settings and "condid_b" in settings and
                      "condid_c" in settings and "spacingid" in settings):
            ...
            if line_data is None:
                if not "phase" in settings.keys():
                    logger.warning(
                        "WARNING:: Skipping Line {} !".format(sectionID))
                continue
            else:
                ...
                if line_data["type"] == "balanced_line":
                    ...
                    try:
                        ...
                        # Two phase line
                            ...
                            a = (1.0 / 3.0 * coeff * complex(
                                2 * float(line_data["r1"]) +
                                float(line_data["r0"]),
                                2 * float(line_data["x1"]) +
                                float(line_data["x0"]),
                            ))
                            b = (1.0 / 3.0 * coeff * complex(
                    ...
                    if ("phasecondid" in line_data and
                            line_data["phasecondid"] in self.conductors):
                        conductor_data = self.conductors[
                            line_data["phasecondid"]]
                    else:
                        conductor_data = {}
                    ...
                    if ("neutralcondid" in line_data and
                            line_data["neutralcondid"] in self.conductors):
                        conductor_data = self.conductors[
                            line_data["neutralcondid"]]
                    else:
                        conductor_data = {}
                ...
                #
                    ...
                    try:
                            ...
                            if p1 == "a" and p2 == "c":
                                impedance_matrix = [
                                    [
                                        ...
                                        coeff * complex(
                                            float(
                                                line_data["mutualresistanceca"]
                                            ),
                                            float(
                                                line_data["mutualreactanceca"]),
                                        ),
                                    ],
                                    [
                                        coeff * complex(
                                ...
                                impedance_matrix = [
                                    [
                                        ...
                                        coeff * complex(
                                            float(line_data[
                                                "mutualresistance{p1}{p2}".
                                                format(p1=p1, p2=p2)]),
                                            float(line_data[
                                                "mutualreactance{p1}{p2}".
                                                format(p1=p1, p2=p2)]),
                                        ),
                                    ],
                                    [
                        ...
                        # Three phase line
                            ...
                            impedance_matrix = [
                                [
                                    coeff * complex(float(line_data["ra"]),
                                                    float(line_data["xa"])),
                                    coeff * complex(
                                ...
                                [
                                    ...
                                    coeff * complex(float(line_data["rb"]),
                                                    float(line_data["xb"])),
                                    coeff * complex(
                                ...
                                [
                                    ...
                                    coeff * complex(float(line_data["rc"]),
                                                    float(line_data["xc"])),
                                ],
                            ]
                    except:
                    ...
                    for phase in phases:
                        if ("condid_{}".format(phase.lower()) in line_data and
                                line_data["condid_{}".format(
                                    phase.lower())].lower() != "none" and
                                line_data["condid_{}".format(
                                    phase.lower())] in self.conductors):
                            conductor_data = self.conductors[line_data[
                                "condid_{}".format(phase.lower())]]
                        else:
                            conductor_data = {}
                    ...
                    if ("condid_n1" in line_data and
                        ...
                        conductor_n1_data = self.conductors[
                            line_data["condid_n1"]]
                        conductor_n2_data = self.conductors[
                            line_data["condid_n2"]]
                    ...
                    elif ("condid_n" in line_data and
                          line_data["condid_n"].lower() != "none" and
                          line_data["condid_n"] in self.conductors):
                        ...
                        if ("condid_n1" in line_data and
                                line_data["condid_n1"].lower() != "none" and
                                line_data["condid_n1"] in self.conductors):
                            conductor_data = self.conductors[
                                line_data["condid_n1"]]
                            api_wire = self.configure_wire(
                                model,
                    ...
                    if settings["spacingid"] in self.spacings:
                        ...
                        for i, p in enumerate(phases):
                            ...
                            for j, k in enumerate(["x", "y"]):
                                if ("posofcond{i}_{k}".format(i=i + 1, k=k)
                                        in spacing_data):
                                    try:
                                        pos[-1][j] = float(spacing_data[
                                            "posofcond{i}_{k}".format(i=i + 1,
                                                                      k=k)])
                                    except:
                                        pass
                        ...
                        if ("posofneutralcond_x" in spacing_data and
                                "posofneutralcond_y" in spacing_data):
                            try:
                                pos[-1][0] = float(
                                    spacing_data["posofneutralcond_x"])
                                pos[-1][1] = float(
                                    spacing_data["posofneutralcond_y"])
                            except:
                                pass

                        pos.append([None, None])

                        if ("posofneutralcond_n2_x" in spacing_data and
                                spacing_data["posofneutralcond_n2_x"] != "" and
                                "posofneutralcond_n2_y" in spacing_data and
                        ...
                        for i, ii in enumerate(valid_cond):
                            ...
                            for j, jj in enumerate(valid_cond):
                                distance_matrix[-1].append(
                                    3.28084 * self.distance(
                                        pos[ii], pos[jj]))  # 0.0328084
                        ...
                        for idx, p in enumerate(phases):
                            if ("condid_{}".format(p.lower()) in settings and
                                    settings["condid_{}".format(
                                        p.lower())] in self.conductors):
                                gmr_list.append(0.0328084 * float(
                                    self.conductors[settings["condid_{}".format(
                                        p.lower())]]["gmr"]))
                                resistance_list.append(1.0 / 0.621371 * float(
                                    self.conductors[settings["condid_{}".format(
                                        p.lower())]]["r25"]))
                            else:
                                logger.warning(
                                    "Could not find conductor {name}. Using DEFAULT..."
                                    .format(name="condid_{}".format(p.lower())))
                                gmr_list.append(
                                    0.0328084 *
                                    float(self.conductors["DEFAULT"]["gmr"]))
                        ...
                        if "condid_n" in settings:
                            if settings["condid_n"] in self.conductors:
                                gmr_list.append(
                                    0.0328084 * float(self.conductors[
                                        settings["condid_n"]]["gmr"]))
                                resistance_list.append(
                                    1.0 / 0.621371 * float(self.conductors[
                                        settings["condid_n"]]["r25"]))
                            else:
                                logger.warning(
                                    "Could not find neutral conductor {name}. Using DEFAULT..."
                                    .format(name=settings["condid_n"]))
                                gmr_list.append(
                                    0.0328084 *
                                    float(self.conductors["DEFAULT"]["gmr"]))
                                resistance_list.append(
                                    1.0 / 0.621371 *
                                    float(self.conductors["DEFAULT"]["r25"]))
                        elif ("condid_n1" in settings and
                              settings["condid_n1"] is not None and
                              settings["condid_n1"].lower() != "none"):
                            if settings["condid_n1"] in self.conductors:
                                gmr_list.append(
                                    0.0328084 * float(self.conductors[
                                        settings["condid_n1"]]["gmr"]))
                                resistance_list.append(
                                    1.0 / 0.621371 * float(self.conductors[
                                        settings["condid_n1"]]["r25"]))
                            else:
                                logger.warning(
                                    "Could not find neutral conductor {name}. Using DEFAULT..."
                                    .format(name=settings["condid_n1"]))
                                gmr_list.append(
                                    0.0328084 *
                                    float(self.conductors["DEFAULT"]["gmr"]))
                                resistance_list.append(
                        ...
                        idx_to_keep = [
                            idx for idx in range(len(distance_matrix))
                    ...
                    for phase in phases:
                        if ("condid_{}".format(phase.lower()) in settings and
                                settings["condid_{}".format(
                                    phase.lower())] in self.conductors):
                            conductor_data = self.conductors[settings[
                                "condid_{}".format(phase.lower())]]
                        else:
                            conductor_data = {}

                        # In addition, we might have some information on the spacings
                        if ("spacingid" in settings and
                                settings["spacingid"] in self.spacings):
                            spacing_data = self.spacings[settings["spacingid"]]
                        else:
                            spacing_data = {}
                    ...
                    # Handle the neutral conductors
                    if ("condid_n" in settings and
                            settings["condid_n"] is not None and
                            settings["condid_n"] != "" and
                            settings["condid_n"] != "NONE" and
                            settings["condid_n"] in self.conductors):
                        conductor_data = self.conductors[settings["condid_n"]]
                    elif ("condid_n1" in settings and
                          settings["condid_n1"] is not None and
                          settings["condid_n1"] != "" and
                          settings["condid_n1"] != "NONE" and
                          settings["condid_n1"] in self.conductors):
    ...
    def parse_capacitors(self, model):
        ...
        for line in self.content:
            ...
            self.capacitors.update(
                self.parser_helper(line, ["serie_capacitor"],
                                   ["id", "reactance"], mapp_serie_capacitor))
        ...
        for sectionID, settings in self.settings.items():
            ...
            try:
                ...
                raise ValueError("Unable to instanciate capacitor {id}".format(
                    id=scap["sectionid"]))
            ...
            if dev_num is not None:
                if dev_num in self.capacitors:
                    ...
                    # Reactance
                    try:
                        api_capacitor.reactance = float(
                            capacitor_data["reactance"])
                    except:
                        pass
            ...
            if "kv" in settings:
                try:
                    if api_capacitor.connection_type == "Y" or len(phases) == 1:
                        api_capacitor.nominal_voltage = (float(settings["kv"]) *
                                                         10**3)  # DiTTo in var
            ...
            if (api_capacitor.pt_phase is not None and
                    api_capacitor.pt_phase not in phases):
                raise ValueError(
                    "Capacitor {name} is monitoring phase {p} which is not in the section {id} phase list {lis}."
                    .format(
            ...
            for p in phases:
                ...
                # Set var value
                if ("fixedkvara" in settings and "fixedkvarb" in settings and
                        "fixedkvarc" in settings and max(
                            float(settings["fixedkvara"]),
                            max(float(settings["fixedkvarb"]),
                                float(settings["fixedkvarc"])),
                        ) > 0):
                    try:
                        if p == "A":
                        ...
                        pass
                elif ("switchedkvara" in settings and
                      "switchedkvarb" in settings and
                      "switchedkvarc" in settings and max(
    ...
    def parse_transformers(self, model):
        ...
        for line in self.content:
            ...
            self.settings.update(
                self.parser_helper(
                    ...
                    [
                        "sectionid", "equipmentid", "connectionconfiguration",
                        "phase"
                    ],
                    mapp_grounding_transformer_settings,
        ...
        for sectionID, settings in self.settings.items():
            ...
            try:
                ...
                raise ValueError(
                    "Unable to instanciate PowerTransformer DiTTo object.")
            ...
            try:
                ...
                raise ValueError(
                    "Empty phases for transformer {}.".format(sectionID))

            # Set from_element
            try:
                api_transformer.from_element = self.section_phase_mapping[
                    sectionID]["fromnodeid"]
            except:
                pass
            ...
            # Set to_element
            try:
                api_transformer.to_element = self.section_phase_mapping[
                    sectionID]["tonodeid"]
            except:
                pass
            ...
            if settings["type"] in [
                ...
                for w in range(3):
                    ...
                    try:
                        ...
                        raise ValueError(
                            "Unable to instanciate Winding DiTTo object.")
                    ...
                    try:
                        ...
                        if w == 1:
                            api_winding.rated_power = (
                                float(settings["secondaryratedcapacity"]) *
            ...
            if settings["type"] == "transformer":
                ...
                matrix = np.matrix([[complex0, 0, 0], [0, complex1, 0],
                                    [0, 0, complex1]])
                a = 1 * cmath.exp(2 * math.pi * 1j / 3)
                T = np.matrix([[1.0, 1.0, 1.0], [1.0, a * a, a],
                               [1.0, a, a * a]])
                T_inv = T.I
                Zabc = T * matrix * T_inv
    ...
    def parse_regulators(self, model):
        ...
        The parse should create the transformers and create separate regulator objects for different phases.
        """
        ...
        mapp_regulators = {
            ...
            "forwardbandwidth": 11,
            "bandwidth":
                11,  # For old CYME version 'forwardbandwidth' is just 'bandwidth'
            "ct": 13,
            "pt": 14,
        }
        ...
        for sectionID, settings in self.settings.items():
            ...
            for p in phases_on:
                ...
                if p not in phases:
                    logger.warning(
                        "Regulator {id} monitors phase {p} which is not in the section phases {pp}"
                        .format(id=sectionID, p=p, pp=phases))
                ...
                try:
                    api_regulator.from_element = self.section_phase_mapping[
                        sectionID]["fromnodeid"]
                except:
                    pass

                try:
                    api_regulator.to_element = self.section_phase_mapping[
                        sectionID]["tonodeid"]
                except:
                    pass
                ...
                try:
                    if (p == "A" and "bandwidtha" in settings and
                            settings["bandwidtha"] is not None):
                    ...
                    if (p == "B" and "bandwidthb" in settings and
                            settings["bandwidthb"] is not None):
                    ...
                    if (p == "C" and "bandwidthc" in settings and
                            settings["bandwidthc"] is not None):
                ...
                for w in range(2):
                    ...
                    # Set the nominal voltage
                    try:
                        api_winding.nominal_voltage = float(
                            regulator_data["kvln"])
                    except:
                        pass
                    ...
                    try:
                        ...
                        raise ValueError(
                            "Unable to instanciate PhaseWinding object.")
    ...
    def parse_loads(self, model):
        ...
        for sectionID, settings in self.customer_loads.items():
            ...
            if "value1" in settings and "value2" in settings:
                if (float(settings["value1"]) == 0.0 and
                        float(settings["value2"]) == 0.0):
                    p = 0
                    q = 0
                elif value_type == 0:  # P and Q are given
                    try:
                        p, q = float(settings["value1"]), float(
                            settings["value2"])
                    except:
                        logger.warning(
                            "WARNING:: Skipping load on section {}".format(
                                sectionID))
                        continue
                    ...
                    try:
                        p, PF = float(settings["value1"]), float(
                            settings["value2"])
                        if 0 <= PF <= 1:
                        ...
                        logger.warning(
                            "Skipping load on section {}".format(sectionID))
                        continue
                ...
                if p >= 0 or q >= 0:
                        ...
                        if not (fusion and sectionID in self._loads):
                            if connectedkva is not None:
                                api_load.transformer_connected_kva = (
                                    connectedkva * 10**3)  # DiTTo in var
                    ...
                    if not (fusion and sectionID in self._loads):
                        if ("loadtype" in settings and
                                settings["loadtype"] in self.customer_class):
                            load_type_data = self.customer_class[
                                settings["loadtype"]]
                        else:
                            load_type_data = {}
                    ...
                    for ph in phases:
                        ...
                        try:
                            api_phase_load.ppercentcurrent = (
                                float(load_type_data["constantcurrentip"]) /
                                100.0)
                            api_phase_load.qpercentcurrent = (
                                float(load_type_data["constantcurrentiq"]) /
                                100.0)
                            api_phase_load.ppercentpower = (
                                float(load_type_data["constantpowerpp"]) /
                                100.0)
                            api_phase_load.qpercentpower = (
                                float(load_type_data["constantpowerpq"]) /
                                100.0)
                            api_phase_load.ppercentimpedance = (
                                float(load_type_data["constantimpedancezp"]) /
                                100.0)
                            api_phase_load.qpercentimpedance = (
                                float(load_type_data["constantimpedancezq"]) /
                                100.0)
    ...
    def parse_dg(self, model):
        ...
        for line in self.content:
            ...
            self.photovoltaic_settings.update(
                self.parser_helper(
                    ...
                    ["photovoltaic_settings"],
                    [
                        "sectionid", "devicenumber", "eqphase",
                        "ambienttemperature"
                    ],
                    mapp_photovoltaic_settings,
        ...
        for sectionID, settings in self.photovoltaic_settings.items():
            try:
                ...
                raise ValueError(
                    "Unable to instanciate photovoltaic {id}".format(
                        id=sectionID))
            try:
                ...
                api_photovoltaic.feeder_name = self.section_feeder_mapping[
                    sectionID.lower()]
                api_photovoltaics[
                    settings["devicenumber"].lower()] = api_photovoltaic
            except:
                raise ValueError(
                    "Unable to set photovoltaic name for {id}".format(
                        id=sectionID))
        ...
        for sectionID, settings in self.bess_settings.items():
            ...
            if dev_num is not None and dev_num in self.bess:
                ...
                try:
                    api_bess.chargeefficiency = float(
                        bess_data["chargingefficiency"])
                except:
                    pass
            ...
            try:
                api_bess.stored_kWh = (float(settings["initialsoc"]) *
        ...
        for deviceID, settings in self.dg_generation.items():
            deviceID = deviceID.strip("*").lower(
            )  # TODO: Deal with multiple configurations for the same location
            api_photovoltaic = api_photovoltaics[deviceID]

            # Use the default setting if available
            if ("loadmodelname" in settings and
                    settings["loadmodelname"].lower() == "default"):
                try:
        ...
        for deviceID, settings in self.converter.items():
            ...
            if deviceID in api_photovoltaics:
                ...
                try:
                    ...
                    pass
                try:
                    api_photovoltaic.fall_limit = float(
                        settings["powerfalllimit"])
                except:
                    pass
                try:
                    api_photovoltaic.rise_limit = float(
                        settings["powerriselimit"])
                except:
                    pass
                ...
                try:
                    api_bess.rated_power = float(
                        settings["activepowerrating"]) * 1000
                except:
                    pass
                try:
        ...
        for deviceID, settings in self.converter_settings.items():
            ...
            if deviceID in api_photovoltaics:
                ...
                try:
                    ...
                    if (api_photovoltaic.control_type
                            == "voltvar_watts_over_vars" or
                            api_photovoltaic.control_type
