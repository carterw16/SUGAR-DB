    ...
    def write_nodes(self, model, fp, sourcebus="sourcebus"):
        ...
        for i in model.models:
            if isinstance(i, Node):
                ...
                # Phases
                if hasattr(i, "phases") and i.phases is not None and len(
                        i.phases) > 0:
                    fp.write("    phases ")
    ...
    def write_transformer_configurations(self, model, fp):
        ...
        for i in model.models:
            if isinstance(i, PowerTransformer):
                ...
                n_windings = 0
                if (hasattr(i, "windings") and i.windings is not None and
                        len(i.windings) > 1):
                    winding1 = i.windings[
                        0]  # Assume winding1 is the primary and winding2 is the secondary unless otherwise stated
                    winding2 = i.windings[1]
                    logger.debug(winding1.nominal_voltage,
                                 winding2.nominal_voltage)
                    if len(i.windings) == 3:
                        dic["connect_type"] = "SINGLE_PHASE_CENTER_TAPPED"

                    elif (hasattr(winding1, "connection_type") and
                          winding1.connection_type is not None and
                          hasattr(winding2, "connection_type") and
                          winding2.connection_type is not None):
                        conn_type = ""
                    ...
                    if (hasattr(winding1, "nominal_voltage") and
                            winding1.nominal_voltage is not None):
                        if (hasattr(winding1, "voltage_type") and
                                winding1.voltage_type == 2):
                            dic["secondary_voltage"] = winding1.nominal_voltage
                        else:
                            dic["primary_voltage"] = winding1.nominal_voltage

                    if (hasattr(winding2, "nominal_voltage") and
                            winding2.nominal_voltage is not None):
                        if (hasattr(winding2, "voltage_type") and
                                winding2.voltage_type == 0):
                            dic["primary_voltage"] = winding2.nominal_voltage
                        else:
                            dic["secondary_voltage"] = winding2.nominal_voltage

                    if (hasattr(winding1, "rated_power") and
                            winding1.rated_power is not None):
                        dic["power_rating"] = winding1.rated_power / 1000.0

                    n_windings = len(i.windings)
                else:
                    logger.debug(
                        "Warning - No windings included in the transformer")

                if hasattr(i, "reactances") and i.reactances is not None:
                    if len(i.reactances) == 1 and n_windings == 2:
                        dic["reactance"] = i.reactances[0]
                        dic["resistance"] = (i.windings[0].resistance +
                                             i.windings[1].resistance)
                ...
                if dic_set in self.transformer_configurations:
                    ...
                    self.transformer_configurations_name[
                        i.name] = self.transformer_configurations[dic_set]
                    continue
                self.transformer_configurations[
                    dic_set] = "transformer_config_{num}".format(
                        num=configuration_count)
                dic["name"] = "transformer_config_{num}".format(
                    num=configuration_count)
                self.transformer_configurations_name[
                    i.name] = "transformer_config_{num}".format(
                        num=configuration_count)
        ...
        for i in model.models:
            if isinstance(i, PowerTransformer):
                ...
                if hasattr(i, "windings") and i.windings is not None:
                    for w in i.windings:
                        if (hasattr(w, "phase_windings") and
                                w.phase_windings is not None):
                            for pw in w.phase_windings:
                                if hasattr(pw,
                                           "phase") and pw.phase is not None:
                ...
                if (hasattr(i, "name") and i.name is not None and
                        i.name in self.transformer_configurations_name):
        ...
        for i in model.models:
            if isinstance(i, Regulator):
                ...
                if (hasattr(i, "connected_transformer") and
                    ...
                    for j in model.models:
                        if (isinstance(j, PowerTransformer) and
                                j.name == i.connected_transformer):
                            if hasattr(j,
                                       "windings") and j.windings is not None:
                                for w in j.windings:
                                    if (hasattr(w, "phase_windings") and
                                            w.phase_windings is not None):
                                        for pw in w.phase_windings:
                                            if (hasattr(pw, "phase") and
                                                    pw.phase is not None):
                                                ...
                                                if (hasattr(
                                                        pw, "compensator_r") and
                                                        pw.compensator_r
                                                        is not None):
                                                    self.regulator_phases[
                                                        dic_set][
                                                            "compensator_r_setting_{phase}"
                                                            .format(
                                                                phase=pw.phase
                                                            )] = pw.compensator_r
                                                if (hasattr(
                                                        pw, "compensator_x") and
                                                        pw.compensator_x
                                                        is not None):
                                                    self.regulator_phases[
                                                        dic_set][
                        ...
                        if (hasattr(w, "phase_windings") and
                            ...
                            for pw in w.phase_windings:
                                if hasattr(pw,
                                    ...
                                    if (hasattr(pw, "compensator_r") and
                                            pw.compensator_r is not None):
                                        self.regulator_phases[dic_set][
                                            "compensator_r_setting_{phase}".
                                            format(phase=pw.phase
                                                  )] = pw.compensator_r
                                    if (hasattr(pw, "compensator_x") and
                                            pw.compensator_x is not None):
                                        self.regulator_phases[dic_set][
                                            "compensator_r_setting_{phase}".
                                            format(phase=pw.phase
                                                  )] = pw.compensator_r
                ...
                if dic_set in self.regulator_configurations:
                    ...
                    self.regulator_configurations_name[
                        i.name] = self.regulator_configurations[dic_set]
                    continue
                self.regulator_configurations[
                    dic_set] = "regulator_config_{num}".format(
                        num=configuration_count)
                dic["name"] = "regulator_config_{num}".format(
                    num=configuration_count)
                self.regulator_configurations_name[
                    i.name] = "regulator_config_{num}".format(
                        num=configuration_count)
                configuration_count = configuration_count + 1
        ...
        for i in model.models:
            if isinstance(i, Regulator):
                if (hasattr(i, "from_element") and
                        ...
                        hasattr(i, "to_element") and i.to_element is not None):
                ...
                if (hasattr(i, "name") and i.name is not None and
                        i.name in self.regulator_configurations_name):
                    fp.write("    configuration {config};\n".format(
            ...
            for i in model.models:
                if isinstance(i, Line):
                    if (hasattr(i, "is_switch") and
                            i.is_switch == 1) or (hasattr(i, "is_fuse") and
                                                  i.is_fuse == 1):
                        continue
                    dic = {}
                    ...
                    if hasattr(i, "wires") and i.wires is not None:
                        for w in i.wires:
                            if (hasattr(w, "phase") and w.phase is not None and
                                    w.phase != "N"):
                    ...
                    if (hasattr(i, "impedance_matrix") and
                        ...
                        if len(phases) != len(lc):
                            logger.debug(
                                "Warning - impedance matrix size different from number of phases for line {ln}"
                                .format(ln=i.name))
                            logger.debug(i.name, i.from_element, i.to_element)
                            logger.debug(phases)
                            logger.debug(lc)
                        for j_cnt in range(
                            ...
                            for k_cnt in range(len(phases)):
                                ...
                                if ("+" not in impedance and
                                        not len(pattern.findall(impedance)) > 0
                                   ):
                                    impedance = "0+" + impedance
                                dic["z{one}{two}".format(
                                    one=phase_map[j_val],
                                    two=phase_map[k_val])] = impedance
                    ...
                    if dic_set in self.line_configurations:
                        self.line_configurations_name[
                            i.name] = self.line_configurations[dic_set]
                        continue

                    self.line_configurations[
                        dic_set] = "line_config_{num}".format(
                            num=configuration_count)
                    dic["name"] = "line_config_{num}".format(
                        num=configuration_count)
                    self.line_configurations_name[
                        i.name] = "line_config_{num}".format(
                            num=configuration_count)
    ...
    def write_lines(self, model, fp):
        for i in model.models:
            if isinstance(i, Line):
                ...
                if (hasattr(i, "line_type") and i.line_type is not None and
                        i.line_type == "underground"):
                    fp.write("object underground_line{\n")
                    if hasattr(i, "length") and i.length is not None:
                        fp.write("    length {len};\n".format(len=i.length *
                                                              3.28084))
                    if (hasattr(i, "name") and i.name is not None and
                            i.name in self.line_configurations_name):
                        fp.write("    configuration {config};\n".format(
                            config=self.line_configurations_name[i.name]))

                elif hasattr(
                        i,
                        "is_fuse") and i.is_fuse is not None and i.is_fuse == 1:
                    fp.write("object fuse{\n")
                elif (hasattr(i, "is_switch") and i.is_switch is not None and
                      i.is_switch == 1):
