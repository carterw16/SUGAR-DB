            ...
            if isinstance(i, Line):
                if hasattr(i, "from_element") and hasattr(i, "to_element"):
                    ...
                    if (hasattr(i, "impedance_matrix") and
                        ...
                        for j_cnt in range(
                            ...
                            for k_cnt in range(len(phases)):
                                ...
                                if len(lc) == 0:
                                    all_z[j][
                                        k] = 0  # Default 0 impedance if no impedance matrix
                                else:
                    ...
                    if (hasattr(i, "capacitance_matrix") and
                        ...
                        if len(phases) != len(lc):
                            logger.debug(
                                "Warning - capacitance matrix size different from number of phases for line {ln}"
                                .format(ln=i.name))
                            logger.debug(i.name, i.from_element, i.to_element)
                            logger.debug(phases)
                            logger.debug(lc)
                    ...
                    logger.warning(
                        "Line missing from and to elements. Nothing written for line {}"
                        .format(i.name))
                    """
                    dic = {}
                    phase_map = {'A': 1, 'B': 2, 'C': 3, '1': 1, '2': 2}
