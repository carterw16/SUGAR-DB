...
import math
import os
from builtins import round, super
...
class Writer(AbstractWriter):
    ...
    **Output file names:**
    ...
    def __init__(self, **kwargs):
        ...
        if "linecodes_flag" in kwargs and isinstance(kwargs["linecodes_flag"],
                                                     bool):
    ...
    def switch(self):
        ...
        for line in self._lines:
            if hasattr(line, "is_switch") and line.is_switch == 1:
                ...
                if (hasattr(line, "from_element") and
                        line.from_element is not None and
                        hasattr(line, "to_element") and
                        line.to_element is not None):
                    logger.debug(
                        " bus1={from_el}".format(from_el=line.from_element))
                    if hasattr(line, "wires") and line.wires is not None:
                        for wire in line.wires:
                            if (hasattr(wire, "phase") and
                                ...
                                obj_dict["From Bus"].append(line.from_element +
                                                            "_" +
                                                            wire.phase.lower())
                                obj_dict["To Bus"].append(line.to_element +
                                                          "_" +
                                                          wire.phase.lower())
                                obj_dict["Switch Name"].append(
                                    line.name + "_" + wire.phase.lower())
                                if hasattr(wire,
                                           "is_open") and wire.is_open == 1:
    ...
    def line(self):
        ...
        for line in self._lines:
            ...
            if hasattr(line, "length") and line.length is not None:
                logger.debug(
                    " Length={length}".format(length=self.convert_from_meters(
            ...
            if hasattr(line, "from_element") and line.from_element is not None:
                ...
                if hasattr(line, "wires") and line.wires is not None:
                    ...
                    for wire in line.wires:
                        if (hasattr(wire, "phase") and
                                wire.phase is not None and wire.phase != "N"):
                            # logger.debug('.{p}'.format(p=self.phase_mapping(wire.phase)))
            ...
            if (hasattr(line, "faultrate") and line.faultrate is not None and
                    not self.linecodes_flag):
            ...
            if hasattr(
                    line,
                    "impedance_matrix") and line.impedance_matrix is not None:
                ...
                for rc, row in enumerate(R):
                    for ec, elt in enumerate(row):
                        ...
                        result += "{e} ".format(e=self.convert_from_meters(
                            np.real(elt), units, inverse=True))
                    result += "| "
                result = result[:
                                -2]  # Remove the last "| " since we do not need it
                result += ") "
            ...
            if (hasattr(line, "capacitance_matrix") and
                    line.capacitance_matrix is not None):
                ...
                for rc, row in enumerate(C):  # Should only be a real matrix
                    for ec, elt in enumerate(row):
                        ...
                        if num_str in valid:
                            ...
                            if elt != 0:
                                B = np.real(
                                    self.convert_from_meters(
                                        elt, units, inverse=True) * 1e-3 * 2 *
                                    60 * math.pi)
                ...
                for rc, row in enumerate(C):
                    for ec, elt in enumerate(row):
                        ...
                        if num_str in valid:
                            ...
                            logger.debug(
                                (np.real(elt) * line.length) * 0.000621371)
                            logger.debug((np.real(elt) * line.length) *
                                         0.000621371 * 1e6)
                            B = (np.real(elt) * line.length * 0.000621371) * 1e6

                            B = np.real(elt) * line.length
                            logger.debug("line units ", B)
                            B = self.convert_from_meters(np.real(B),
                                                         units,
                                                         inverse=False)
                            B = B * 1e6
        ...
        for i in self._transformers:
            ...
            if len(i.windings[0].phase_windings) == 1:
                ...
                if from_element_name in self._transformer_dict:
                    ...
                    self._transformer_dict[from_element_name][
                    ...
                    self._transformer_dict[i.from_element + i.name] = {
                        ...
                        "i":
                            index,
                        "kv1":
                            i.windings[0].nominal_voltage,
                        "Tap " + pp:
                            i.windings[0].phase_windings[0].tap_position,
                        "kva":
                            i.windings[0].rated_power,
                    }
            else:
                index += 1
                for key, value in obj_dict.items():
        ...
        for i in self._transformers:
            ...
            if len(i.windings) >= 2:
                ...
                obj_dict["X (pu)"][index] = i.reactances[
                    0]  # TODO check currently opendss reads in reactances is defined as [value1, value2, ...] for each winding type. May need to change.
                if hasattr(i, "windings") and i.windings is not None:
                    N_phases = []
                    for winding_num, winding in enumerate(i.windings):
                        if winding_num > 1:
                            break
                        if (i.from_element in self._transformer_dict and
                                self._transformer_dict[
                                    i.from_element]["combined"]):
                            obj_dict["W" + str(winding_num + 1) +
                                     "S_base (kVA)"][index] = round(
                                         kva / 1000, 2)
                            obj_dict["W" + str(winding_num + 1) +
                                     "V (kV)"][index] = round(
                                         (kv1 / math.sqrt(3)) / 1000, 2)
                        else:
                            obj_dict["W" + str(winding_num + 1) +
                                     "S_base (kVA)"][index] = (
                        ...
                        obj_dict["W" + str(winding_num + 1) +
                                 "R (pu)"][index] = (winding.resistance / 100)
            ...
            if hasattr(i, "loadloss") and i.loadloss is not None:
                logger.debug(" %loadloss=" +
                             str(i.loadloss))  # opendss in kWatts
        ...
        for index, i in enumerate(self._powersources):
            ...
            for phase_unicode in i.phases:
                ...
                if str(ph).lower() in set(["a", "b", "c"]):
                    obj_dict["bus " +
                             str(ph).upper()][index] = (i.connecting_element +
                                                        "_" + str(ph).lower())
    ...
    def bus(self):
        ...
        [i for i in self.m.models if isinstance(i, Node)]
        index = 0
                    ...
                    if phase == "a" or phase == "b" or phase == "c":
                        ...
                        if isinstance(node.nominal_voltage, float):
                            obj_dict["Voltage (V)"].append(
                                ...
                                math.sqrt(3))  # L-N voltage
                        else:
                            obj_dict["Voltage (V)"].append(
                                node.nominal_voltage)  # PAG why is this needed
                        if (node.name.lower() == "sourcebus"
    ...
    def load(self):
        ...
        for x, i in enumerate(loads):
            ...
            if hasattr(i, "name") and i.name is not None:
                ...
                if (i.name[-1].lower() == "a" or i.name[-1].lower() == "b" or
                        i.name[-1].lower() == "c"):
                    n_name = i.name[0:-1]
                        ...
                        if hasattr(j, "use_zip") and j.use_zip is not None:
                            if (
                                ...
                                logger.debug(
                                    ...
                                    format(
                                        phase=j.phase,
                                        cf=(j.ppercentcurrent +
                                            j.qpercentcurrent),
                                    ))
                                logger.debug(
                                    "    current_pf_{phase} {cpf};\n".format(
                                        phase=j.phase,
                                        cpf=(j.ppercentcurrent /
                                             (j.ppercentcurrent +
                                              j.qpercentcurrent)),
                                    ))
                                logger.debug(
                                ...
                                logger.debug(
                                    ...
                                    format(
                                        phase=j.phase,
                                        iff=(j.ppercentimpedance +
                                             j.qpercentimpedance),
                                    ))
                                logger.debug(
                                    "    impedance_pf_{phase} {ipf};\n".format(
                                        phase=j.phase,
                                        ipf=(j.ppercentimpedance /
                                             (j.ppercentimpedance +
                                              j.qpercentimpedance)),
                                    ))
                                logger.debug(
                                ...
                                obj_dict["P_" + str(phase_cnt) +
                                         " (kW)"][index] = (j.p / 1000.0)
                                obj_dict["Q_" + str(phase_cnt) +
                                         " (kVAr)"][index] = (j.q / 1000.0)
                                obj_dict["K_p"][index] = (j.ppercentpower +
                                                          j.qpercentpower)
                                obj_dict["K_i"][index] = (j.ppercentcurrent +
                                                          j.qpercentcurrent)
                                obj_dict["K_z"][index] = (j.ppercentimpedance +
                                                          j.qpercentimpedance)
                                ...
                                if (hasattr(j, "p") and j.p is not None and
                                        hasattr(j, "q") and j.q is not None and
                                        hasattr(j, "phase") and
                                        j.phase is not None):
                                    logger.debug(
            ...
            if i.connection_type == "Y":
                ...
                self.logger.error(
                    "Unsupported type of connection {conn} for Load {name}".
                    format(conn=i.connection_type, name=i.name))

            obj_dict["Status"][index] = int(1)  # this doesn't work right
            obj_dict["Use initial voltage?"][index] = int(
                0)  # this doesn't work right
    ...
    def write(self, m, **kwargs):
        ...
        self._capacitors = [
            i for i in self.m.models if isinstance(i, Capacitor)
        ]
        self._transformers = [
            i for i in self.m.models if isinstance(i, PowerTransformer)
        ]
        self._regulators = [
            i for i in self.m.models if isinstance(i, Regulator)
        ]
        self._powersources = [
            i for i in self.m.models if isinstance(i, PowerSource)
        ]
        self._photovoltaics = [
            i for i in self.m.models if isinstance(i, Photovoltaic)
        ]
...
if __name__ == "__main__":
    ...
    _reader.set_dss_file_names({
        "Nodes":
            "../../validation/inputs/opendss/ieee_13_node/IEEE13Node_BusXY.csv"
    })
