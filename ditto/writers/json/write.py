    ...
    def write(self, model):
        ...
        for obj in model.models:
            ...
            for key, value in obj._trait_values.items():
                if key in [
                        "capacitance_matrix", "impedance_matrix", "reactances"
                ]:
                    ...
                    for v in value:
                        ...
                        if isinstance(v, complex):
                            ...
                            for phw in v.phase_windings:
                                json_dump["model"][-1][key]["value"][-1][
                                    "phase_windings"]["value"].append(
                                        {"class": "PhaseWinding"})
                                for kkkk, vvvv in phw._trait_values.items():
                                    json_dump["model"][-1][key]["value"][-1][
                                        "phase_windings"]["value"][-1][kkkk] = {
                                            "class":
                                                str(type(vvvv)).split("'")[1],
                                            "value":
                                                vvvv,
                                        }
