...
from .base import DiTToHasTraits, Float, Instance, Int, Unicode
...
class Timeseries(DiTToHasTraits):
    ...
    data_label = Unicode(
        help=
        "The label assigned to the dataset. This describes what the name should be when it is outputted to through the writers"
        "",
        default_value=None,
    )
    interval = Float(
    ...
    data_type = Unicode(
        help=
        """This is the python datatype of the timeseries e.g. float, complex etc.""",
        default_value=None,
    )
    loaded = Int(
        help=
    ...
    scale_factor = Float(
        help=
        """A number to multiply the entire timeseries by for scaling purposes""",
        default_value=1,
    )
