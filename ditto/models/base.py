...
from traitlets.traitlets import (EventHandler, ObserveHandler, _CallbackWrapper,
                                 _deprecated_method)
...
class DiTToHasTraits(T.HasTraits):
    ...
    response = T.Any(allow_none=True,
                     help="default trait for managing return values")
    ...
    def set_name(self, model):
        try:
            ...
            if name in model.model_names:
                warnings.warn(
                    "Duplicate name %s being set. Object overwritten." % name)
                logger.debug(
                    "Duplicate name %s being set. Object overwritten." % name)
