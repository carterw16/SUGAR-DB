...
class Node(DiTToHasTraits):
    ...
    positions = List(
        Instance(Position),
        help=
        """This parameter is a list of positional points describing the node - it should only contain one.
    ...
    is_substation_connection = Bool(
        help=
        """1 if the node connects from inside a substation to outside, 0 otherwise.""",
        default=None,
    )
    ...
    setpoint = Float(
        help=
        """Value that the node must be set to. This is typically used for feeder head points""",
        default_value=None,
    )
