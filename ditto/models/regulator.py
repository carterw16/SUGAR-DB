...
class Regulator(DiTToHasTraits):
    ...
    delay = Float(help="""The delay for first tap change operation""",
                  default_value=None)
    numtaps = Int(help="""Total number of taps between min and max tap""",
                  default_value=32)
    mintap = Float(help="""Mininmum effective regulator tap value""",
                   default_value=0.9)
    maxtap = Float(help="""Maximum effective regulator tap value""",
                   default_value=1.1)
    highstep = Int(
    ...
    pt_ratio = Float(
        help=
        """The turns ratio used for the power transducer with a line-drop compensator.""",
        default_value=None,
    )
    ct_ratio = Float(
        help=
        """The turns ratio used for the current transducer with a line-drop compensator.""",
        default_value=None,
    )
    phase_shift = Float(
    ...
    bandcenter = Float(
        help=
        """This is the target value for the regulator. Should often be the nominal voltage""",
        default_value=None,
    )
    voltage_limit = Float(
    ...
    connected_transformer = Unicode(
        help=
        """The name of the transformer that the voltage regulator is attached to""",
        default_value=None,
    )
    pt_phase = Unicode(help="""The phase being used to monitor the voltage""",
                       default_value=None)
    positions = List(
        Instance(Position),
        help=
    ...
    ct_prim = Float(
        help=
        """Rating, in Amperes, of the primary CT rating for converting the line amps to control amps""",
        default_value=None,
    )
    ...
    noload_loss = Float(
        help=
        """The no-load loss for a zero sequence short-circuit test on the regulator""",
        default_value=None,
    )
