...
class PowerTransformer(DiTToHasTraits):
    ...
    install_type = Unicode(
        help=
        """The mounting type of the transformer: one of {POLETOP, PADMOUNT, VAULT}""",
        default_value=None,
    )
    noload_loss = Float(
        help=
        """The no-load loss for a zero sequence short-circuit test on the entire transformer""",
        default_value=None,
    )
    phase_shift = Float(
    ...
    from_element = Any(
        help=
        """Name of the node which connects to the 'from' end of the transformer""",
        default_value=None,
    )
    to_element = Any(
        help=
        """'Name of the node which connects to the 'to' end of the transformer""",
        default_value=None,
    )
    ...
    windings = List(
        Instance(Winding),
        help=
        """A list of the windings that the transformer contains. Most will have two windings but center tap transformers have three.""",
        default_value=None,
    )
    positions = List(
        Instance(Position),
    ...
    normhkva = Float(help="Normal maximum kVA rating for H winding",
                     default_value=None)
    ...
    is_substation = Bool(
        help=
        """Set to 1 if the transformer is a substation or is inside a substation""",
    )
