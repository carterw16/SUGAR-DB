...
class Network:
    ...
    def build(self, model, source="sourcebus"):
        ...
        for i in model.models:
            ...
            if (hasattr(i, "from_element") and i.from_element is not None and
            ...
            if hasattr(
                    i,
                    "connecting_element") and i.connecting_element is not None:
                a = len(graph_nodes)
        ...
        edge_equipment_name = nx.get_edge_attributes(self.graph,
                                                     "equipment_name")
        for edge in edge_equipment:
            if self.digraph.has_edge(*edge):
                self.digraph[edge[0]][
                    edge[1]]["equipment"] = edge_equipment[edge]
            elif self.digraph.has_edge(*edge[::-1]):
                self.digraph[edge[1]][
                    edge[0]]["equipment"] = edge_equipment[edge]

        for edge in edge_equipment_name:
            if self.digraph.has_edge(*edge):
                self.digraph[edge[0]][
                    edge[1]]["equipment_name"] = edge_equipment_name[edge]
            elif self.digraph.has_edge(*edge[::-1]):
                self.digraph[edge[1]][
                    edge[0]]["equipment_name"] = edge_equipment_name[edge]
    ...
    def rebuild_digraph(self, model, source="sourcebus"):
        ...
        graph_edges = set(self.digraph.edges()
                ...
                if (hasattr(i, "from_element") and
                    ...
                    if (i.from_element, i.to_element) in graph_edges:
                        for attr in tuple(
                            ...
                            if attr[0] != "_":
                                self.digraph[i.from_element][
                                    i.to_element][attr] = getattr(i, attr)
                    ...
                    if (i.to_element, i.from_element) in graph_edges:
                        for attr in tuple(
                            ...
                            if attr[0] != "_":
                                self.digraph[i.to_element][
                                    i.from_element][attr] = getattr(i, attr)
                ...
                if (hasattr(i, "connecting_element") and
                    ...
                    if (i.connecting_element, i.name) in graph_edges:
                        for attr in tuple(
                            ...
                            if attr[0] != "_":
                                if self.digraph.has_edge(
                                        i.connecting_element, i.name):
                                    self.digraph[i.connecting_element][
                                        i.name][attr] = getattr(i, attr)
                                else:
                                    self.digraph.add_edge(i.connecting_element,
                                                          i.name,
                                                          length=0)
                                    self.digraph[i.connecting_element][
                                        i.name][attr] = getattr(i, attr)
                ...
                if (hasattr(i, "connecting_element") and
                    ...
                    if (i.connecting_element, i.name) in graph_edges:
                        for attr in tuple(
                            ...
                            if attr[0] != "_":
                                if self.graph.has_edge(i.connecting_element,
                                                       i.name):
                                    self.graph[i.connecting_element][
                                        i.name][attr] = getattr(i, attr)
                                else:
                                    self.graph.add_edge(i.connecting_element,
                                                        i.name,
                                                        length=0)
                                    self.graph[i.connecting_element][
                                        i.name][attr] = getattr(i, attr)
    ...
    def remove_open_switches(self, model):
        for m in model.models:
            if (hasattr(m, "is_switch") and
                (m.is_switch or m.is_breaker) is not None and
                    hasattr(m, "from_element") and hasattr(m, "to_element") and
                    m.from_element is not None and m.to_element is not None and
                    len(m.wires) > 0):
                is_open = True
                for w in m.wires:
                    if (hasattr(w, "is_open") and w.is_open is not None and
                            not w.is_open):
                        is_open = False
    ...
    def get_upstream_transformer(self, model, node):
        ...
        edge_equipment_name = nx.get_edge_attributes(self.digraph,
                                                     "equipment_name")
    ...
    def get_all_elements_downstream(self, model, source):
        ...
        #
        nx.get_edge_attributes(self.graph, "equipment")
        edge_equipment_name = nx.get_edge_attributes(self.graph,
                                                     "equipment_name")
        ...
        for x in _elts:
            try:
                ...
                raise ValueError(
                    "Unable to get DiTTo object with name {}".format(x))
    ...
    def middle_single_phase(self, nodes):
        ...
        for i in range(len(nodes)):
            ...
            if (not "phases" in self.graph.nodes[nodes[i]]
               ) or self.graph.nodes[nodes[i]]["phases"] == None:
        ...
        for i in range(len(nodes)):
            node = nodes[i]
            if at_1p_section and len(
                    self.graph.nodes[node]["phases"]) > min_phase:
                if cnt > max_cnt:
                    max_cnt = cnt
                    pos_max_cnt = i - 1
                cnt = 0
        ...
        if pos_max_cnt > -1:
            order1 = tuple([
                nodes[pos_max_cnt - max_cnt / 2],
                nodes[pos_max_cnt - max_cnt / 2 + 1]
            ])
            order2 = tuple([
                nodes[pos_max_cnt - max_cnt / 2 + 1],
                nodes[pos_max_cnt - max_cnt / 2]
