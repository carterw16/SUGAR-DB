...
class system_structure_modifier(Modifier):
    ...
    def __init__(self, model, *args):
        ...
        for x in self.model.models:
            if (isinstance(x, PowerSource) and hasattr(x, "nominal_voltage") and
                    x.nominal_voltage is not None and x.is_sourcebus):
    ...
    def set_missing_coords_recur(self):
        ...
        for i in self.model.models:
            if (hasattr(i, "positions") and
                (i.positions is None or len(i.positions) == 0 or
                 i.positions[0].lat == 0 or i.positions[0].long == 0) and
                (hasattr(i, "name") and i.name is not None)):
                if (i.name in self.G.graph.nodes()
                   ):  # Only find coords of missing nodes - not edges
                    recur_nodes.append(
                        i.name)  # Should be passing the reference to the node
        ...
        while len(recur_nodes) > 0:
            ...
            for i in recur_nodes:
                ...
                for j_name in self.G.graph.neighbors(i):
                    j = self.model[j_name]
                    if (hasattr(j, "positions") and j.positions is not None and
                            len(j.positions) != 0 and
                            j.positions[0].lat != 0 and
                            j.positions[0].long != 0):
                        adj_lats_longs.append(
                            (j.positions[0].lat, j.positions[0].long))
            ...
            if len(next_recur) == len(recur_nodes):
                for i in recur_nodes:
                    logger.warning(
                        "Unable to compute coordinates for {}".format(i))
                    print("Unable to compute coordinates for {}".format(i))
                return
            recur_nodes = next_recur

    def set_feeder_metadata(self,
                            feeder_name=None,
                            substation=None,
                            transformer=None):
            ...
            if isinstance(obj, Feeder_metadata):
                ...
                if name_cleaned in headnodes:
                    ...
                    if name_cleaned in cleaned_headnodes:
                        obj.headnode = headnodes[cleaned_headnodes.index(
                            name_cleaned)]
                    else:
                        reverse_headnodes = []
                        ...
                        if name_cleaned in reverse_headnodes:
                            obj.headnode = headnodes[reverse_headnodes.index(
                                name_cleaned)]
                            obj.nominal_voltage = self.model[
                                obj.headnode].nominal_voltage
    ...
    def set_nominal_voltages_recur(self, *args):
        ...
        if (previous, node) in self.edge_equipment and self.edge_equipment[(
                previous, node)] == "PowerTransformer":
            ...
            new_value = min([
                ...
                node, previous)] == "PowerTransformer":
        ...
        for obj in self.model.models:
            ...
            if isinstance(obj, Line) and obj.nominal_voltage is None:
                ...
                if hasattr(obj,
                    ...
                    if (hasattr(node_from_object, "nominal_voltage") and
                            node_from_object.nominal_voltage is not None):
    ...
    def set_load_coordinates(self, **kwargs):
        ...
        Default values are:
            ...
            - delta_elevation=0
        """
        if "delta_longitude" in kwargs and isinstance(kwargs["delta_longitude"],
                                                      (int, float)):
            delta_longitude = kwargs["delta_longitude"]
        else:
            delta_longitude = 0.1

        if "delta_latitude" in kwargs and isinstance(kwargs["delta_latitude"],
                                                     (int, float)):
            delta_latitude = kwargs["delta_latitude"]
        else:
            delta_latitude = 0.1

        if "delta_elevation" in kwargs and isinstance(kwargs["delta_elevation"],
                                                      (int, float)):
        ...
        for obj in self.model.models:
            if isinstance(obj, Load) and obj.positions is None:
                if (hasattr(obj, "connecting_element") and
                    ...
                    try:
                        if (hasattr(self.model[obj.connecting_element],
                                    "position") and
                                self.model[obj.connecting_element].position
                                is not None):
                            position_obj = self.model[
                                obj.connecting_element].positions
                            obj.positions = []
    ...
    def feeder_preprocessing(self):
        ...
        for elt in self.model.models:
            if isinstance(elt, PowerTransformer):
                ...
                if (hasattr(elt, "is_substation") and elt.is_substation == 1 and
                        hasattr(elt, "name")):
                    ...
                    for down_elt in downstream_elts:
                        if (hasattr(down_elt, "is_substation") and
                                down_elt.is_substation == 1):
                            logger.debug(
                                "Info: substation {a} found downstream of substation {b}"
                                .format(b=elt.name, a=down_elt.name))
                            skip = True
                            break
                    ...
                    if not skip:
                        ...
                        for down_elt in downstream_elts:
                            if (down_elt.substation_name is not None and
                                    len(down_elt.substation_name) != 0):
                                raise ValueError(
                                    "Substation name for element {name} was already set at {_previous}. Trying to overwrite with {_next}"
                                    .format(
                                        name=down_elt.name,
                            ...
                            if (down_elt.feeder_name is not None and
                                    len(down_elt.feeder_name) != 0):
                                raise ValueError(
                                    "Feeder name for element {name} was already set at {_previous}. Trying to overwrite with {_next}"
                                    .format(
                                        name=down_elt.name,
    ...
    def replace_kth_switch_with_recloser(self):
        ...
        for elt in self.model.models:
            ...
            if (hasattr(elt, "is_substation_connection") and
                    elt.is_substation_connection):

                switch_cnt = 0
                target_depth = np.random.choice([1, 2, 3], p=[0.7, 0.25, 0.05])
                if (hasattr(elt, "nominal_voltage") and
                        elt.nominal_voltage < 69000
                    ...
                    while should_continue:
                        ...
                        try:
                            end_node = next(
                                self.G.digraph.successors(from_node))
                        except:
                            break
                        ...
                        if (from_node, end_node) in self.edge_equipment_name:
                            _name = self.edge_equipment_name[(from_node,
                                                              end_node)]
                        elif (end_node, from_node) in self.edge_equipment_name:
                            _name = self.edge_equipment_name[(end_node,
                                                              from_node)]
                        ...
                        if (hasattr(_obj, "is_switch") and
                                _obj.is_switch == 1 and
                                hasattr(_obj, "wires") and
                                len(_obj.wires) > 0 and
                                not _obj.wires[0].is_open):
                            switch_cnt += 1
            ...
            for j, f2 in enumerate(self._list_of_feeder_objects):
                if j > i:
                    ...
                    if len(intersection) != 0:
                        logger.debug("=" * 40)
                        logger.debug(
                            "Feeder {n} and feeder {m} intersect:".format(n=i,
                                                                          m=j))
    ...
    def set_nominal_voltages(self):
        ...
        **Algorithm:**
        ...
        edges_to_remove = [
            ...
            if "equipment" in edge[2] and
            edge[2]["equipment"] == "PowerTransformer"
        ]
        ...
        for idx, group in enumerate(node_mapping):
            ...
            if upstream_transformer_name is not None:
                ...
                if (hasattr(upstream_transformer_object, "windings") and
                        upstream_transformer_object.windings is not None):
        ...
        for idx, group in enumerate(node_mapping):
            ...
            for n in group:
                ...
                if isinstance(self.model[n], Load):
                    self.model[
                        n].upstream_transformer_name = upstream_transformer_name_group[
                            idx]
                ...
                if hasattr(obj,
                    ...
                    #
                        ...
                        if upstream_transformer_name is not None:
                            upstream_transformer_object = self.model[
                                upstream_transformer_name]
                            if (hasattr(upstream_transformer_object, "windings")
                                    and upstream_transformer_object.windings
                                    is not None):
                                volts = []
                                for winding in upstream_transformer_object.windings:
                                    if (hasattr(winding, "nominal_voltage") and
    ...
    def center_tap_load_preprocessing(self):
            ...
            while continu:
                ...
                if _type == "PowerTransformer":
                    ...
                    if (from_node, end_node) in self.edge_equipment_name:
                        ...
                        self.model[
                            load_list[idx].
                            name].upstream_transformer_name = self.edge_equipment_name[
                                (from_node, end_node)]
                    elif (end_node, from_node) in self.edge_equipment_name:
                        transformer_names.append(
        ...
        for t_name in transformer_names:
            ...
            try:
                ...
                raise ValueError(
                    "Unable to retrieve DiTTo object with name {}".format(
                        t_name))
        ...
        #
        for idx, load, lines, phase in zip(range(len(load_list)), load_list,
                                           line_names, phases):
            ...
            for line in lines:
                ...
                for wire in line_obj.wires:
                    ...
                    if (hasattr(wire, "phase") and
                            wire.phase not in ["N", "N1", "N2"] and
                            wire.phase not in phase):
                        wire.drop = 1
            ...
            for p in phase:
                ...
                # Work on the load...
                if p not in [
                        phase_load.phase for phase_load in load.phase_loads
                ]:
                ...
                for line in lines:
                    try:
                        ...
                        raise ValueError(
                            "Unable to retrieve DiTTo object with name {}".
                            format(line))
            ...
            n_real_phase_loads = sum(
                [1 for pl in load.phase_loads if pl.drop != 1])
            for phase_load in load.phase_loads:
                    ...
                    if _type == "PowerTransformer":
                        ...
                        if (from_node, end_node) in self.edge_equipment_name:
                            transformer_name = self.edge_equipment_name[(
                                from_node, end_node)]
                            self.model[
                                load_name].upstream_transformer_name = self.edge_equipment_name[
                                    (from_node, end_node)]
                        elif (end_node, from_node) in self.edge_equipment_name:
                            transformer_name = self.edge_equipment_name[(
                                end_node, from_node)]
    ...
    def open_close_switches(self, path_to_dss_file):
        ...
        for line in lines:
            ...
            data_line = {
                k.strip().lower(): v.strip() for k, v in zip(
    ...
    def set_switching_devices_ampacity(self):
        ...
        for obj in self.model.models:
            if isinstance(obj, Line):
                if (obj.is_switch is True or obj.is_breaker is True or
                        obj.is_sectionalizer is True or obj.is_fuse is True or
                        obj.is_recloser is True):
                    ...
                    if np.any(np.isnan(amps)):
                        ...
                        # Case 2: All ampacity ratings are nans
                            ...
                            if (obj.from_element is not None and
                                ...
                                while should_continue:
                                    if self.G.graph.has_node(
                                            ...
                                            if (
                                                ...
                                                try:
                                                    ...
                                                    if neighboring_line_obj.wires[
                                                            0].ampacity is not None and not np.isnan(
                                                                neighboring_line_obj
                                                                .wires[0].
                                                                ampacity):
                                        ...
                                        raise ValueError(
                                            "Missing nodes {n1} and/or {n2} in network"
                                            .format(n1=from_node, n2=to_node))
