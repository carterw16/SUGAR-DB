...
from ditto.models.base import (Any, Complex, DiTToHasTraits, Float, Instance,
                               Int, List, Unicode, observe)
...
class Modifier:
    ...
    def delete_element(self, model, obj):
        ...
        for attr in obj.traits():
            class_name = str(type(
                obj.traits()[attr])).strip("<>'").split(".")[-1]
            if class_name == "List":
                if (
                        getattr(obj, attr) is None or
                        getattr(obj, attr) is None or
                        len(getattr(obj, attr)) == 0 or attr == "reactances" or
                        attr == "phases" or attr == "impedance_matrix" or
                        attr == "capacitance_matrix"
        ...
        model.remove_element(
            obj)  # TODO: Improve this so the data isn't stored in a list
        return model
    ...
    def set_attributes(self, model_1, obj_1, obj_2, overwrite=True):
        ...
        for (attr) in (
            ...
            class_name = str(type(
                obj_2.traits()[attr])).strip("<>'").split(".")[-1]
            ...
            if class_name == "List":
                ...
                for i in range(len(list_1)):
                    if not (hasattr(list_1[0], "phase") and
                            list_1[0].phase is not None):
                        has_phases = False
                for i in range(len(list_2)):
                    if not (hasattr(list_2[0], "phase") and
                            list_2[0].phase is not None):
                        has_phases = False
                if has_phases and len(list_1) > 0 and len(list_2) > 0:
                    ...
                    for i in range(len(list_1)):
                        self.set_attributes(model_1, list_1[i], list_2[i],
                                            overwrite)
    ...
    def merge(self, model_1, model_2, overwrite=True):
        ...
        for (obj_name) in (
            ...
            if (obj_name in model_1.model_names
    ...
    def add(self, model_1, model_2):
        ...
        for obj_name in model_2.model_names:
            self.copy(model_1, model_2.model_names[obj_name])
