        ...
        for row in all_rows:
            ...
            if len(entries) > 0 and entries[0] == "object":
                if curr_object is None:
                    obj = entries[1].split(":")
                    obj_class = obj[0]
                    if (obj_class == "house" or obj_class == "solar" or
                            obj_class == "inverter" or
                            obj_class == "waterheater" or
        ...
        for obj_name, obj in self.all_gld_objects.items():
            ...
            if obj_type == "node" or obj_type == "meter":
                ...
                try:
                    ...
                    for i in obj["phases"][
                            0:
                            -1]:  # Ignore the 'N' at the end and just say if A,B or C
            ...
            if obj_type == "triplex_node" or obj_type == "triplex_meter":
                ...
                try:
                    ...
                    for i in obj["phases"][
                            1:
                            -1]:  # Ignore the 'S' at the end and just say if A,B or C
                        # With lists of traitlets, the strings aren't automatically cast
                        cnt = cnt + 1
                    ...
                    if phase == "A":
                        ...
                        try:
                            ...
                            complex_voltage = complex(
                                obj["voltage_A"])  # Needed to compute the power
                            if (
                                ...
                                p = (((complex_voltage.real**2 +
                                       complex_voltage.imag**2)) /
                                     complex_impedance.conjugate()).real
                                q = (((complex_voltage.real**2 +
                                       complex_voltage.imag**2)) /
                                     complex_impedance.conjugate()).imag
                                phaseload.p = p
                                phaseload.q = q
                                phaseload.use_zip = 0
                                phaseload.model = (
                        ...
                        try:
                            ...
                            complex_voltage = complex(
                                obj["voltage_A"])  # Needed to compute the power
                            p = (complex_voltage *
                                 complex_current.conjugate()).real
                            q = (complex_voltage *
                                 complex_current.conjugate()).imag
                            if (
                        ...
                        try:
                            ...
                            complex_voltage = complex(
                                obj["voltage_B"])  # Needed to compute the power
                            if (
                                    complex_impedance.real != 0
                            ):  # Assume that unless ZIP is specifically specified only one of constant_power, constant_current and constant_impedance is used. A real part of 0 means that I or P is being used instead.
                                p = (((complex_voltage.real**2 +
                        ...
                        try:
                            ...
                            complex_voltage = complex(
                                obj["voltage_C"])  # Needed to compute the power
                            if (
                                    complex_impedance.real != 0
                            ):  # Assume that unless ZIP is specifically specified only one of constant_power, constant_current and constant_impedance is used. A real part of 0 means that I or P is being used instead.
                                p = (((complex_voltage.real**2 +
                ...
                if not impedance_matrix_direct:
                    ...
                    for i in range(len(impedance_matrix)):
                        for j in range(len(impedance_matrix[0])):
                            impedance_matrix[i][
                                j] = impedance_matrix[i][j] / 1609.34
